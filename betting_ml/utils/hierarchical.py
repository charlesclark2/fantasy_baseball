...
⭐ SHARED, CROSS-VERTICAL HOME (promoted here by MLB Edge-E7.3, 2026-07-26). This is the ONE
partial-pooling solver the whole program reuses: NCAAF-P1.2 (team strength), P1.2b (freshman
MLE), P1A (college→NFL MLE), and now baseball E7.3 (MiLB→MLB minor-league-equivalency MLE)
all fit their random-slopes/intercepts model through it. The football module
`quant_sports_intel_models/football/ncaaf/models/hierarchical.py` is now a thin re-export
shim over this file, so there is a single implementation, not per-vertical copies. It is pure
numpy at import (scipy is imported lazily inside the solvers), so it is fast-gate-safe.

Sport-agnostic linear algebra. Nothing in this module knows what football (or baseball) is; it knows how
