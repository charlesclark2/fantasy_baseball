...
def _coerce_numeric(X: pd.DataFrame) -> pd.DataFrame:
    ...
    object columns → numeric. mu/spread/uncertainty NaN (missing signal groups)
    is preserved — LightGBM handles it natively; Ridge/GLM median-impute it. Used
...
def assign_r(mu: np.ndarray, edges: np.ndarray, r_per_bin: np.ndarray, global_r: float) -> np.ndarray:
    """Map each predicted mu to its decile r (global_r fallback)."""
...
# ---------------------------------------------------------------------------
# Champion artifact — picklable; exposes predict_mu_r(X) for Stories 10.3 / 10.6
# ---------------------------------------------------------------------------
...
class TotalsNegBinModel:
    ...
    def predict_mu(self, X: pd.DataFrame) -> np.ndarray:
        mu = self.mean_model.predict(_coerce_numeric(X[self.feature_cols]))
