"""
ablation_matchup_v1_signals.py — Story 8.4

Incremental ablation: does adding matchup_advantage_mu_v1 and
matchup_volatility_signal_v1 improve totals and run-differential CV MAE?

Compares walk-forward temporal CV (2015+ data, min_train_seasons=3):
  - Baseline:     feature_pregame_game_features columns only
  - With signals: + home_matchup_advantage_mu_v1, away_matchup_advantage_mu_v1,
                    home_matchup_volatility_signal_v1, away_matchup_volatility_signal_v1

Matchup signals are available from 2021+ only (94%+ coverage); 2015-2020 rows
and unavailable rows are filled with neutral values (mu=0.0, volatility=mean).
The model sees a "no-info" value for years outside the signal window, so the
...
Usage:
    uv run python betting_ml/scripts/ablation_matchup_v1_signals.py
    uv run python betting_ml/scripts/ablation_matchup_v1_signals.py --dry-run
"""
...
# Neutral fill for pre-2021 rows and unavailable game-sides:
#   mu=0.0  — signal is a residual; zero means no detectable matchup edge
#   volatility=1.756 — mean entropy from 2021-2025 backfill (matches "average" uncertainty)
_MU_NEUTRAL         = 0.0
_VOLATILITY_NEUTRAL = 1.756

_SIGNAL_COLS = [
    "home_matchup_advantage_mu_v1",
    "away_matchup_advantage_mu_v1",
    "home_matchup_volatility_signal_v1",
    "away_matchup_volatility_signal_v1",
]

_SIGNAL_QUERY = """
select
    game_pk,
    max(case when side = 'home' then matchup_advantage_mu_v1      end) as home_matchup_advantage_mu_v1,
    max(case when side = 'away' then matchup_advantage_mu_v1      end) as away_matchup_advantage_mu_v1,
    max(case when side = 'home' then matchup_volatility_signal_v1 end) as home_matchup_volatility_signal_v1,
    max(case when side = 'away' then matchup_volatility_signal_v1 end) as away_matchup_volatility_signal_v1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Story 8.4 ablation: matchup_v1 signals")
    parser.add_argument("--dry-run", action="store_true",
                        help="Print data shape and signal coverage then exit.")
    args = parser.parse_args()

    print("=== STORY 8.4 — MATCHUP V1 SIGNALS ABLATION TEST ===\n")
    ...
    print("\nLoading matchup_v1 signals from feature_pregame_sub_model_signals...")
    ...
    # Fill pre-2021 and unavailable rows with neutral values
    for col in ["home_matchup_advantage_mu_v1", "away_matchup_advantage_mu_v1"]:
        df_merged[col] = df_merged[col].fillna(_MU_NEUTRAL)
    for col in ["home_matchup_volatility_signal_v1", "away_matchup_volatility_signal_v1"]:
        df_merged[col] = df_merged[col].fillna(_VOLATILITY_NEUTRAL)
...
""")
        ...
        mu_rank  = next((i+1 for i, (n, _) in enumerate(ranked) if "matchup_advantage_mu" in n and "home" in n), None)
        vol_rank = next((i+1 for i, (n, _) in enumerate(ranked) if "matchup_volatility" in n and "home" in n), None)

        print(f"  Feature importance (Ridge |coef|, {target}):")
        print(f"    home_matchup_advantage_mu_v1:      rank #{mu_rank} of {len(ranked)}")
        print(f"    away_matchup_advantage_mu_v1:      rank #{next((i+1 for i,(n,_) in enumerate(ranked) if 'matchup_advantage_mu' in n and 'away' in n), None)} of {len(ranked)}")
        print(f"    home_matchup_volatility_signal_v1: rank #{vol_rank} of {len(ranked)}")
    ...
    if any_gate:
        ...
        print("  Near-zero delta is expected: matchup_advantage_mu encodes")
        print("  batter/pitcher interaction residuals already partially captured")
        print("  by raw features. True payoff is Epic 9 stacking.")

    _REPORT_DIR.mkdir(parents=True, exist_ok=True)
    run_ts = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    report_path = _REPORT_DIR / "matchup_v1_ablation.md"

    lines = [
        "# Story 8.4 — Matchup v1 Signal Ablation Results",
        "",
        f"**Run date:** {run_ts}  ",
        f"**Script:** `betting_ml/scripts/ablation_matchup_v1_signals.py`  ",
        f"**Signal coverage:** 2021–2026 (94%+ available; 2015–2020 rows filled with neutral values: mu={_MU_NEUTRAL}, volatility={_VOLATILITY_NEUTRAL})  ",
    ...
    for target, res in target_results.items():
        ...
        lines += [
            ...
            f"- `home_matchup_advantage_mu_v1` Ridge |coef| rank: #{res['mu_rank']}",
            f"- `home_matchup_volatility_signal_v1` Ridge |coef| rank: #{res['volatility_rank']}",
            "",
        ]
