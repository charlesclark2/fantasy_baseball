"""
regenerate_starter_oos.py — Leakage fix (starter_v1 suppression walk-forward OOS signals)

Per game-side starter suppression signal (LightGBM mean + Normal sigma),
leakage-free. Mirrors the offense template (idx-based prepare_fold). Data floor
2016 (EB xwOBA), so trains from 2016. See `project_layer3_signal_leakage`.

Per eval season S: per-fold Optuna (inner walk-forward of train seasons), fit
LightGBM on [train_from..S-1], fit Normal sigma on train residuals, predict S.
Emit per (game_pk, side): starter_suppression_mu, starter_suppression_sigma,
starter_uncertainty (80% Normal PI width = 2·1.2816·sigma).

Output: betting_ml/models/layer3/oos_signals/oos_signals_starter.parquet
        columns: game_pk, side, season, starter_suppression_mu,
                 starter_suppression_sigma, starter_uncertainty

Usage:
    uv run python betting_ml/scripts/leakage_fix/regenerate_starter_oos.py --trials 30
"""
...
from betting_ml.scripts.starter_v1.train_starter_v1 import (
    load_data, prepare_fold, fit_sigma, normal_nll, calib_80,
)

_OUT_PATH = _PROJECT_ROOT / "betting_ml" / "models" / "layer3" / "oos_signals" / "oos_signals_starter.parquet"
_TRAIN_FROM_DEFAULT = 2016
_EVAL_FROM_DEFAULT = 2021
_SEED = 42
_PI80 = 2.0 * 1.2816  # 80% Normal PI width factor
...
def _tune(df_train: pd.DataFrame, inner_folds: list[tuple], trials: int) -> dict:
    ...
    def objective(trial) -> float:
        ...
        for tr_idx, ev_idx in inner_folds:
            ...
            mu_tr, mu_ev = m.predict(X_tr), m.predict(X_ev)
            nlls.append(normal_nll(y_ev, mu_ev, fit_sigma(y_tr, mu_tr)))
...
def regenerate(df: pd.DataFrame, train_from: int, eval_from: int, trials: int) -> pd.DataFrame:
    ...
    for S in eval_seasons:
        ...
        mu_tr, mu_ev = model.predict(X_tr), model.predict(X_ev)
        sigma = fit_sigma(y_tr, mu_tr)
        nll, c80 = normal_nll(y_ev, mu_ev, sigma), calib_80(y_ev, mu_ev, sigma)
        ...
        print(f"  eval {S}: train {train_from}-{S-1} ({df_train['game_year'].nunique()} seasons, "
              ...
              f"calib80={c80:.3f}  sigma={sigma:.4f}")

        sub = df.loc[eval_idx, ["game_pk", "side", "game_year"]].reset_index(drop=True)
        sub = sub.rename(columns={"game_year": "season"})
        sub["starter_suppression_mu"] = mu_ev
        sub["starter_suppression_sigma"] = float(sigma)
        sub["starter_uncertainty"] = float(_PI80 * sigma)
        out.append(sub)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Leakage-free walk-forward starter_v1 suppression OOS signals")
    ...
    print("Loading starter suppression data (load_data)...")
    df = load_data()
    ...
    print(f"\nWrote {len(oos):,} OOS starter-suppression rows → {args.out}")
    print(oos.groupby("season").agg(n=("game_pk", "size"),
                                    mean_mu=("starter_suppression_mu", "mean")).round(4).to_string())
