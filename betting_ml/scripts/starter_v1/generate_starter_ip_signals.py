...
Usage:
    ...
    uv run python betting_ml/scripts/starter_v1/generate_starter_ip_signals.py --date 2026-06-01 --env dev
"""
...
_TABLE_NAME   = "starter_ip_signals"
...
def _resolve_target(env: str) -> str:
    ...
    prod → baseball_data.betting_features.starter_ip_signals
    dev  → baseball_data.dev_betting_features.starter_ip_signals
    """
...
# ---------------------------------------------------------------------------
# E11.1-W9-tail: read the starter-IP feature sources (mart_starting_pitcher_game_log,
# feature_pregame_starter_features, stg_statsapi_games — all in S3 post-W8b) from the
# S3 lakehouse via DuckDB. The MERGE WRITE to starter_ip_signals stays on Snowflake
# (the W9 export-mirror copies that OUTPUT to S3; re-implementing MERGE accumulate in
# DuckDB is the W7a-wipe class W9 forbids). Reuses scripts.utils.lakehouse_read. The
# only SF dialect token is IFF() → IF() (DuckDB has IF, not IFF); game_date is DATE in
# the parquet → no ::date cast needed.
# ---------------------------------------------------------------------------
...
def _load_games_s3(start_date: str, end_date: str):
    ...
    sql = _SCORE_QUERY.format(start_date=start_date, end_date=end_date)
