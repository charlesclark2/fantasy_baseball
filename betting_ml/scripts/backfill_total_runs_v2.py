"""Card 7.V Task 4 — backfill v2 total_runs predictions.

Loads ngboost_tuned_v2.pkl, scores every 2021+ game with finalized results
in mart_game_results, and inserts/upserts per-game rows into
baseball_data.betting_ml.daily_model_predictions with model_version='v2'.

Only the total_runs columns are populated:
...
home_win and run_diff columns are intentionally left NULL — v2 only changes
the total_runs model.

Idempotent: skips (game_pk, model_version='v2') pairs that already exist
unless --force is passed (which deletes existing v2 rows first).

Run from project root:
    uv run python betting_ml/scripts/backfill_total_runs_v2.py
    uv run python betting_ml/scripts/backfill_total_runs_v2.py --force
    uv run python betting_ml/scripts/backfill_total_runs_v2.py --dry-run
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backfill v2 total_runs predictions.")
    parser.add_argument("--force", action="store_true",
                        help="Delete existing v2 rows before inserting (full rewrite).")
    parser.add_argument("--dry-run", action="store_true",
                        help="Score and report counts but do not write to Snowflake.")
    args = parser.parse_args()

    print(f"Loading v2 artifact: {_ARTIFACT_PATH}")
    ...
    print("Loading 2021+ historical features (joined with mart_game_results)...")
    df = load_features(min_games_played=15)
    print(f"  Loaded {len(df):,} rows; seasons {sorted(df['game_year'].unique())}")

    # Build raw feature matrix and apply the same imputation pipeline used in training.
    raw_cols = [c for c in feature_cols if c in df.columns or c.startswith(("has_", "is_"))]
    # The pipeline materialises has_starter_platoon_data and is_new_venue indicators,
    # so feed it the same raw column set the training script used.
    ...
    try:
        cur = conn.cursor()
        if args.force:
            cur.execute(_DELETE_V2_SQL, {"model_version": _MODEL_VERSION})
            print(f"  --force: deleted {cur.rowcount or 0} existing v2 row(s)")
            existing: set[int] = set()
        else:
            cur.execute(_EXISTING_PKS_SQL, {"model_version": _MODEL_VERSION})
            existing = {int(r[0]) for r in cur.fetchall() if r[0] is not None}
            print(f"  Existing v2 rows: {len(existing)}; will skip those game_pks")
        ...
        # Insert in chunks to keep transactions manageable
        chunk = 1000
        n_inserted = 0
        ...
        conn.commit()
        print(f"  Committed {n_inserted} v2 rows.")
    finally:
        conn.close()
