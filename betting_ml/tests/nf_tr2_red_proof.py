...
BREAKS = [
    ...
    ("build-time fit: the unqualified `veteran_tier_size` NameError the first cut shipped", _RSP,
     "        panel, level_form, projection_season, _SP.veteran_tier_size(),",
     "        panel, level_form, projection_season, veteran_tier_size(),",
     "build_time_level_fit_is_invoked"),
    ("build-time fit: never invoked (the helper returns the identity with the policy ON)", _RSP,
