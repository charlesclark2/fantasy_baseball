#!/usr/bin/env python3
"""
scripts/ddl/generate_w11c_external_tables.py   (E11.1-W11 Tier-C — weather ingestion → S3)

Emit Snowflake EXTERNAL TABLE DDL over the S3 parquet that the W11c DuckDB build writes, so the
weather dbt models' Snowflake (else) branch `select * from baseball_data.lakehouse_ext.<model>`
resolves. The 4 weather models are TABLE/VIEW-materialized on the Snowflake side (no incrementals) →
no DROP+rebuild is needed at cutover (unlike the W8a EB posteriors).

The W11c parquet is written by:
  scripts/run_w1_lakehouse.py --w11c   (each model → lakehouse/<model>/data.parquet)
which reads the weather_raw raw mirror written by:
  ingest_weather / backfill_observed_weather dual-writing under W11_RAW_WRITE_MODE=both/s3, and
  scripts/export_w11_raw_to_s3.py --source weather_raw  (the one-time history bridge).

PREREQUISITES:
  1. run_w1_lakehouse.py --w11c (or --w11c-only) has written all 4 parquet files.
...
USAGE:
  uv run python scripts/ddl/generate_w11c_external_tables.py            # → stdout + .sql file
  uv run python scripts/ddl/generate_w11c_external_tables.py --print    # stdout only

OUTPUT: scripts/ddl/w11c_external_tables.generated.sql — REVIEW, then run in Snowflake BEFORE the
PR merges (the else branches read these). Refresh: refresh_w1_external_tables.py (--w11c / W11C_TABLES).
AUTO_REFRESH=FALSE.
"""
...
OUT_PATH = REPO_ROOT / "scripts" / "ddl" / "w11c_external_tables.generated.sql"
...
# The 4 W11c weather models (single-file data.parquet). Order matches run_w1_lakehouse.W11C_MODELS.
W11C_MODELS = [
    "stg_weather_raw",
...
# Keep this in sync with _string_timestamp_wrap (it stringifies EVERY TIMESTAMP* output col). DATE cols
# read correctly from parquet → NOT listed. feature_pregame_weather_features has NO timestamp outputs.
TS_STRING_COLS = {
    "stg_weather_raw":                {"game_datetime_utc", "loaded_at"},
    "stg_weather_raw_snapshots":      {"loaded_at"},
    "feature_pregame_weather_status": {"valid_from", "valid_to", "computed_at"},
    # feature_pregame_weather_features: game_pk / venue_id / floats / bool — no TIMESTAMP outputs.
}
...
def emit_external_table(model: str, cols) -> str:
    ...
    # The weather parquet columns are lowercase (the stg/feature duckdb branches alias every column
    ...
    return (
        ...
        f"COMMENT = 'E11.1-W11 Tier-C: {model} from S3 lakehouse parquet (weather feed)';\n"
...
def main() -> None:
    ...
    for model in W11C_MODELS:
        try:
            ...
            print(f"  SKIP {model}: {e}  (have you run run_w1_lakehouse.py --w11c yet?)",
                  file=sys.stderr)
    conn.close()

    verify = "\n-- Verification:\n" + "".join(f"-- SELECT count(*) FROM {SCHEMA}.{m};\n" for m in W11C_MODELS)
    ddl = "\n".join(blocks) + verify
    if print_only:
        print(ddl)
    else:
