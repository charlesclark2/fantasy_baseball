#!/usr/bin/env bash
# Deploy the Cognito PreSignUp auto-link trigger Lambda (E9.7).
#
# This packages ONLY handler.py — boto3 ships in the Lambda Python runtime, so there
# are no dependencies to bundle. The function CODE is created/updated here; the IAM
# role, the Cognito trigger wiring, and the one-time duplicate cleanup are one-time
# operator steps documented in README.md (run those FIRST for a fresh deploy).
#
# Usage:
#   ./infrastructure/cognito/presignup_link/deploy.sh              # update code (function must exist)
#   ./infrastructure/cognito/presignup_link/deploy.sh --dry-run    # zip only
#
...
FUNCTION_NAME="${PRESIGNUP_FUNCTION_NAME:-credence-prod-cognito-presignup-link}"
...
echo "=== Cognito PreSignUp link Lambda deploy ==="
