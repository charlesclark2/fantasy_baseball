...
| `E11_24_UMPIRE_REBUILD_GATE`, `E11_24_BULLPEN_S3_READ` | **both `0` today; flip to `1` in a QUIET WINDOW** | E11.24 **target 6** — the intraday EB/lineup chain. Both ship default-OFF and are in `env.required` for **PRESENCE only** (no value pinned), so a rollback cannot false-page. ⛔ **Flip OUTSIDE a live slate** (before ~06:00 or after ~23:30 CST) — INC-36 made mid-slate deploys the risk window. **`E11_24_UMPIRE_REBUILD_GATE`** = skip the intraday umpire dbt rebuild when the HP-umpire assignment has not changed since the last rebuild (measured **111 of 662 provisioning waits / 8d = 16.8%**, the largest remaining `COMPUTE_WH` waker; UTC hours 13-23). Fail-OPEN on every path; the once-daily `dbt_umpire_feature_rebuild` stays UNGATED as a floor. **`E11_24_BULLPEN_S3_READ`** = `update_team_posteriors` reads its bullpen observations from the S3 lakehouse via DuckDB instead of Snowflake (⚠️ NOT fail-open by design — it feeds a non-idempotent ordered chain). 🔒 **Verification is a REAL box run, not CI** (CI mocks all IO): after the flip, confirm `predict_today` shows **no new abstains and no new absent\|all-null contracts** vs the pre-flip slate, then measure **resumes AND active-minutes IN THE 14–23 BAND** (`uv run python scripts/report_e11_24_wake_census.py` — use **7/28 and 7/30** as the pre-flip reference; 7/31 is contaminated by the prior session's audit queries and **8/1 by INC-37 remediation**). ⭐ **AND run `uv run python scripts/check_w11_tail_coverage.py --date <slate>`** — the six-block coverage gate does NOT measure umpire/weather/public betting, so it read a healthy 0.878 in INC-37 while two blocks were empty; the 8/1 pre-flip reference is umpire 5/15 PARTIAL (a pre-existing nightly-`--w11b` lag), weather 14/15 OK, public_betting 15/15 OK. 🔎 6a's blast radius is smaller than the flag text implies: `lineup_dbt_feature_rebuild` COPIES the SF external table — the SERVED umpire parquet comes from the nightly `--w11b`, which 6a does not touch. **Confirmed 2026-08-02:** the umpire `lakehouse_ext` tables are refreshed ONLY by `refresh_w1_external_tables.py --w11b` (an early-return path called only by the nightly W11b mirror op, and absent from the default no-arg daily refresh), so across a slate they are FROZEN and the skipped CTAS is byte-identical **by construction**. 🚨 **…AND SO IS ITS EXPECTED SAVING — re-measured pre-flip, the gate's premise is FALSE.** "Assignment written exactly ONCE per slate" does not hold: 13 slates (07-20..08-01) carry a **median of 8 distinct same-day `loaded_at` instants (range 7–20)**, because `ingest_umpires.py --skip-if-exists` is gated `and do_sf` (SF-leg-only) while the box runs `W11_RAW_WRITE_MODE=s3` ⇒ the guard never executes ⇒ every post-assignment tick re-writes and bumps the watermark. Sized by INVOCATIONS not executions (the lever-1b lesson): umpire-rebuild FIRES in the 14–23 band vs same-day watermark BUMPS, 07-22..08-01, are median **12 vs 8** ⇒ ratio ~1.5 ⇒ ~30–35% cut. **⚠️⚠️ SUPERSEDED 2026-08-04 (FU-3 verification session): the ~1.5 ratio was an OVER-COUNT** (it counted "fires" as 5-min windows containing an umpire CTAS — double-counting a rebuild spanning a window boundary, and later windows hold a lineup CTAS with no umpire CTAS = phantom skips; `%..umpire_game_log%` also matched the trailing dbt `GRANT`). The clean instrument = **executions of `feature_pregame_umpire_features`** (once per un-gated invocation; validates to FU-1's independently-recorded 7 rebuilds on 08-02). Re-derived 07-25→08-02 **median ratio = 1.00 ⇒ 6a ALONE IS ~INERT** (pre-FU-3 every tick re-wrote the mirror + bumped the watermark, so the gate's key was ALWAYS satisfied). ⇒ **6a's entire saving is contingent on FU-3; expect the observed cut to track FU-3's −27% write-instant reduction, NOT ~30–35%.** Unlocking it requires `--skip-if-exists` to work on the S3 leg AND per-game rather than any-row (an any-row check would swallow a later-announced assignment) — a SEPARATE change, deliberately not bundled with this flip. ✅ **FU-3 + 6a DEPLOYED (dev→main) 2026-08-04 06:04 UTC — LIVE + ARMED** (verified in the persistent `dagster-codeloc` container: no `and do_sf`, recreated `StartedAt=06:04:38Z`, image digest == deploy-log digest, `E11_24_UMPIRE_REBUILD_GATE=1`). ⚠️ Before 08-04 every slate ran the pre-FU-3 SF-leg-only guard — the FU-3 prompt's "already on main" premise was FALSE. FU-3 replay 07-20→08-03 (15 slates): **131 write-instants → 95, −27%, median 8→6**; wave-count floor confirmed (survivors sit at/one-above each slate's announcement-wave count; every slate ≥3 distinct `first_seen`). 🚨 **STILL UNVERIFIED LIVE — the soak's FIRST armed normal slate = 2026-08-04 (15 games).** ⛔ target-6 is NOT unblocked until the 08-04 verification is (a)+(b) green. 08-04 STRADDLES MIDNIGHT UTC (first pitches 22:35Z 08-04→01:40Z 08-05) ⇒ both UTC-day-bucketed censuses undercount it — run the box event-log read for BOTH `2026-08-04` AND `2026-08-05` and combine (earliest ~02:00Z 08-05; census ~14:00Z 08-05). Full recap: story_prompts.md (FU-3 block) + `docs/e11_24_literal_zero_snowflake.md`. ⏭️ Do NOT promote either to the §10a enforced (`=='1'`) set on its first soak. |
