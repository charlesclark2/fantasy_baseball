...
> Re-run of E7.9's retrain on the **2016–2026** window (11 seasons ⇒ **8 folds**) with a **pre-registered 4-arm family**, under the FIXED DSR convention. Arm of the design: **PRIMARY (2020 kept)**.
...
- Honest metric **crps** (lower = better) · 4 arms × 8 purged/embargoed folds · 21,006 rows · seed 42
- Incumbent arm `incumbent::ngboost_normal` = 2.5205
- Leader `plus_eb::glm_elasticnet` = 2.4908 (margin +0.0297; noise floor 0.02)
- PBO 0.010 (gate < 0.2) · DSR 1.000 (gate ≥ 0.95)
...
- **Window** `2016–2026` — 11 seasons ⇒ **8 folds** (E7.9 ran 6 seasons ⇒ 3). Arm: **PRIMARY (2020 kept)**.
...
| **this run** | 8 | 4 | **1.182** | 0.9999 | True |
...
|---|---|---:|---|---:|---|
| **FIXED** (per-fold + measured `trial_sharpes`) | purged folds | 8 | measured, 0.09167 | **1.000** | ✅ **YES** |
| legacy (E7.9 as recorded) | year-month buckets | 46 | asymptotic `1/n_obs` | 1.000 | no — reported only |

Leader's per-fold skill series (incumbent − leader, positive ⇒ leader better): mean `+0.03598`, SD `0.03443`, Sharpe `1.045` against a deflated benchmark `SR0 = 0.319`.

Trial Sharpes (non-reference arms — the incumbent IS the reference, so its identically-zero skill series is excluded from `V` per `h_harness.dsr_report`): `{'incumbent::glm_elasticnet': 0.842, 'plus_eb::ngboost_normal': 0.4493, 'plus_eb::glm_elasticnet': 1.0448}`.

**Sensitivity on the benchmark, so a high bar can be told from a broken one:** at the asymptotic `V = 1/n_obs` (the benchmark MH2 §7's design table used) the same series gives `SR0 = 0.372` and **DSR 1.000**, against the measured-`V` `SR0 = 0.319` / DSR 1.000. The MEASURED figure BINDS, as pre-registered — this line is disclosure, not a re-pick.

PBO is reported on BOTH surfaces: **0.010** on year-month buckets (pre-registered as binding, so the wide window stays comparable to E7.9 on this gate) and 0.000 at the coarser fold level.
...
|---:|---:|---:|---:|---|---:|---:|---:|
| 1 | 2019 | 2,142 | 0.835 | `away_lineup_bat_speed_vs_starter_velo`, `home_starter_proj_fip` | 2.5993 | 2.5888 | -0.0106 |
| 2 | 2020 | 583 | 0.908 | `away_lineup_bat_speed_vs_starter_velo` | 2.5763 | 2.4683 | -0.1080 |
| 3 | 2021 | 2,090 | 0.910 | `away_lineup_bat_speed_vs_starter_velo` | 2.5226 | 2.4576 | -0.0650 |
| 4 | 2022 | 2,149 | 0.914 | `away_lineup_bat_speed_vs_starter_velo` | 2.4668 | 2.4509 | -0.0160 |
| 5 | 2023 | 2,151 | 0.950 | — | 2.5364 | 2.5152 | -0.0212 |
| 6 | 2024 | 2,146 | 0.990 | — | 2.4318 | 2.3929 | -0.0389 |
| 7 | 2025 | 2,025 | 0.995 | — | 2.5611 | 2.5427 | -0.0184 |
| 8 | 2026 | 1,362 | 0.990 | — | 2.5086 | 2.4989 | -0.0098 |
...
|---|---:|---:|
| **learner swap** (incumbent learner → `glm_elasticnet`) | +0.0175 | 59% |
| **contract** (added features) | +0.0122 | 41% |
| **total reported margin** | +0.0297 | 100% |

🚩 **59% of this margin is the LEARNER SWAP, not the features.** Do not read `+0.0297` as what the added columns bought — that figure is `+0.0122`.
...
|---|---:|---:|---:|---:|
| glm_elasticnet | 2.5030 | n/a | +0.0122 | n/a |
| ngboost_normal | 2.5205 | n/a | +0.0122 | n/a |
...
Calibration tolerance in effect: `0.00717` (pre-registration amendment #1, 2026-07-29 — max of a 0.001 absolute floor and 10% of the incumbent's PIT-KS).
...
|---|---|---|---:|---:|---:|
| `oracle_floor` | oracle_floor | - | 0.0002 | 0.5000 | 14,648 |
| `plus_eb::glm_elasticnet` | plus_eb | glm_elasticnet | 2.4908 | 0.0618 | 14,648 |
| `incumbent::glm_elasticnet` | incumbent | glm_elasticnet | 2.5030 | 0.0662 | 14,648 |
| `plus_eb::ngboost_normal` | plus_eb | ngboost_normal | 2.5084 | 0.0718 | 14,648 |
| `incumbent::ngboost_normal` | incumbent | ngboost_normal | 2.5205 | 0.0717 | 14,648 |
