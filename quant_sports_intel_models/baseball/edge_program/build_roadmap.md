...
| 1 | **E9.10** (A0.4.11) 🔄 | Settings: profile + prefs (**unblocked parts only**) | **P1** | **CURRENT** — ⚠️ PARTIAL → **scope = finish the unblocked parts only**: profile, sign-out / sign-out-everywhere, non-email prefs, and the **editable `initial_deposit`** E9.17 needs. Beta-launch blocker. The **email/push notif toggle is explicitly OUT of this card** → deferred to E9.9 + SES. |
| 2 | **E9.5** (A0.4.22) | Password reset flow | **P1** | 🟢 **READY (SES prod live 2026-06-18)** — sandbox-validated; finish: branded template + redeploy + retest with a real email. Quick beta-launch win. |
| 3 | **E9.4** (A0.4.18) | Welcome email + beta onboarding | **P1** | 🟢 **UNBLOCKED (SES prod live)** — the literal gate to inviting beta users; brand the invite template + provision. Resend Path B retired. |
...
| 9 | **E9.17** | Bankroll-growth % + editable deposit | P2 | Feature + settings; **needs E9.10's deposit field**. |
