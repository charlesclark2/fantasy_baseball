...
> **BUILT + PRE-CHECKED (2026-06-23) — lift run handed to operator.** Built `home_/away_bullpen_fatigue_short_leash = bullpen_pitches_prev_3d::float / nullif(starter_avg_ip_last_3, 0)` in `feature_pregame_game_features_raw.sql` (eval-only; **no production contract references it**; `::float` division — explicitly NOT bare `::numeric`, per the B1 truncation lesson). Both parents are in the E2.1 per-side allow-list, so the harness reads the candidate as `opp_bullpen_fatigue_short_leash` (the FACED pitching side). **Applying the B1 lesson, the warehouse was checked BEFORE the operator's long run:** (a) **data is well-formed, NOT degenerate** — per-season mean ≈ 33–37, std ≈ 15–26, ~97% non-null coverage, no zero-collapse (2021–2026); (b) **the pre-registered orthogonality gate PASSES** — corr(interaction, pen_pitches) = **0.65**, corr(interaction, starter_ip) = **−0.58** (both < 0.7), parents nearly independent (−0.04) → it carries genuine interaction variance, not a level proxy. So the operator's run is a *real* lift test, not a degenerate/redundant column. Guards: dbt `assert_bullpen_fatigue_short_leash_not_degenerate` + harness `candidate_is_degenerate`. **VERDICT (lift): RULED OUT — clean, trustworthy null on BOTH targets (2026-06-23, after full-refresh).** On the correctly-backfilled column (eval coverage **96.8%** per-side / **97.4%** home_win; not-degenerate; orthogonality 0.68 / 0.59 — genuinely the interaction, both <0.7): **per-side CRPS** lift all −0.0004 / non_cold −0.0011 (slightly hurts), DSR 0.356, PBO 0.700; **home_win logloss** lift all +0.0000 / non_cold −0.0005, DSR 0.518, PBO 0.963. Fails lift+PBO+DSR on both surfaces → **NO-SHIP.** Exactly the pre-registered expectation: the GBMs already capture the bullpen-fatigue × short-leash interaction implicitly via hierarchical splits on the two parents, so the explicit product term adds nothing (its small edge would have lived in a GLM lane, which the per-side/home_win champions are not). Artifacts: `ablation_results/e13_4_b2_fatigue_{perside_runs,home_win}_lift.json`.
>
> **FIRST RUN INVALID (2026-06-23) — incremental build left the column NULL across history.** The handoff said `dbtf build --select …` (plain incremental). But `feature_pregame_game_features(_raw)` are `incremental` with a **7-day lookback** `is_incremental()` filter, so the build only repopulated the last week — `home_/away_bullpen_fatigue_short_leash` came out **0% non-null for 2015–2025 and 10.3% (124 rows) in 2026**, while the parents stayed ~97% populated. Result: per-side LightGBM saw a constant column in training, dropped it → **byte-identical predictions → harness `🛑 DEGENERATE` (guard worked)**; home_win champion saw ~124 non-null rows → produced a non-degenerate-looking but **untrustworthy** "lift −0.0014 / NO-SHIP" on ~nothing (slipped past the std/byte-identical checks). **NEITHER run is a verdict.** This is the same `incremental --full-refresh` propagation gotcha B1 hit and full-refreshed away last session. **Guards hardened this session** (both squarely "model-lift integrity"): the dbt test now also fails on COVERAGE collapse (output null while parent populated), and the harness `candidate_is_degenerate` now flags candidates non-null on < 50% of eval games. **CORRECTED build (must full-refresh the two incremental models so the column backfills across all seasons): `dbtf build --select feature_pregame_game_features_raw feature_pregame_game_features --full-refresh`** (the upstream `mart_starter_tto_splits` / `feature_pregame_starter_features` are `table`s, already full). Then re-run the §6.4 lift commands; the new dbt coverage test should PASS only once the backfill lands.
...
0. **Build the feature — MUST `--full-refresh` the incremental models** (a plain incremental build
   only repopulates the 7-day lookback and leaves the new column NULL across history → invalid lift;
   this happened on the first attempt 2026-06-23):
   `dbtf build --select feature_pregame_game_features_raw feature_pregame_game_features --full-refresh`
   then the CI compile `dbtf compile`. (`mart_starter_tto_splits` / `feature_pregame_starter_features`
   are `table`s, already full.) The hardened `assert_bullpen_fatigue_short_leash_not_degenerate`
   now also fails on COVERAGE collapse — it should PASS only once the backfill lands across all seasons.
