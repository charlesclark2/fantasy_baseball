...
### E7.7 — FanGraphs prospect ingest (THE BOARD → `fg_minor_id`)  🟡 **CODE-COMPLETE (2026-07-27) — pending the operator's ONE box pull**
**Tasks:**
- [x] Ingest FanGraphs **THE BOARD** (Future Value / rank / ETA / level / org) → S3 Delta, keyed by `fg_minor_id`, with `as_of_date` snapshot dating for leakage-safe as-of joins (E7.8). Correctly-tiered Dagster op (WARN — non-serving).
**Endpoint (probed, not coded-to-docs):** FanGraphs is behind a Cloudflare JS challenge (`cf-mitigated: challenge`, HTTP 403 to any direct client — re-verified live 2026-07-27). The board is served by ONE JSON endpoint — `GET https://www.fangraphs.com/api/prospects/board/data?draft=<season>prospect&season=<yr>` (~1,300 prospects/season) — confirmed against the FanGraphs mobile-app path (`github.com/hedgertronic/fungo::get_prospect_board`). We route THROUGH the box's **FlareSolverr** proxy via the existing `scripts/utils/fangraphs_client.py` (new `fetch_prospects_board()`), consistent with every other FG ingest. **⭐ Each row carries BOTH `playerid` (→ `fg_minor_id`) AND `xMLBAMID` (→ the MLBAM id E8.0 joins to the E7.3 MLE projections on)** — no fragile name-match. **Column-name-reality:** every row stores its FULL `raw_json` + typed columns via CASE-INSENSITIVE alias lookup; a `--probe` mode prints the live board's real field names + alias resolution and writes nothing (the ONE real pull confirms the extraction before any write).
**Files:** `scripts/ingest_fangraphs_prospects_to_s3.py` (S3-native Delta, SF-FREE, instance-role auth via `delta_lake.storage_options`; partition `(season, as_of_date)`, idempotent; `--probe`/`--dry-run`/`--from-csv` fallback), `fetch_prospects_board()` in `scripts/utils/fangraphs_client.py`, `pipeline/ops/milb_ops.py::fangraphs_prospects_ingest_op` (WARN-tier) wired into `milb_ingest_job` (daily 13:00 UTC), `scripts/tests/test_fangraphs_prospects_ingest.py` (12).
**Storage:** `s3://baseball-betting-ml-artifacts/baseball/milb/the_board` (Delta), one row per (as_of_date, season, fg_minor_id).
**AC:** ⏳ `fg_minor_id`-keyed prospect parquet with as-of dating + coverage report (# ranked, # with MLBAM id, seasons) — **emitted on the operator's first box pull** (`--probe` then a real write; needs `FLARESOLVERR_URL`). SF-free confirmed. Manual-CSV fallback (`--from-csv`) available if the endpoint is blocked on 8/3.
