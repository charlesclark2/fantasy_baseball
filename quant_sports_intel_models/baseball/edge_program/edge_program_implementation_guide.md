...
### E7.3p — PITCHER minor→major translation (MLE)  🏗️ **HARNESS CODE-COMPLETE (2026-07-27) — awaiting the operator's real S3 run**  **[the E7.3 sibling; closes the pitcher half of E8.0 + the starter-EB cold-start]**
**Tasks:**
...
**AC (pending the real run):** a ≥3-form pitcher bake-off → validated per-level MLE (mean + PARAMETER uncertainty) with per-metric verdicts + graduated-pitcher holdout error, PBO/DSR-guarded; projections landed at `baseball/milb/derived/mle_projections_pitchers` (the E8.0 board's pitcher column); the starter-EB path documented. `best_alpha = 0`. ⏭️ Operator: `build_graduated_pairs_pitchers --season-floor 2015 --s3` then `run_milb_mle_pitchers --pairs … --s3` (both >1-min; see the session handoff).
