...
### E2.5 — Signal registration + leakage-safe backfill  🔄 CODE-COMPLETE (2026-07-26; operator backfill + deploy pending)
**Tasks:**
- [x] New version `totals_generative_v1` → dedicated `totals_generative_signals` table (mirrors `offense_v2_signals`); registered in `sub_model_registry.yaml`.
- [x] Leakage-safe backfill respecting `[[project_layer3_signal_leakage]]` — **walk-forward as-of scoring**: each eval season *Y* scored by a model trained only on seasons `< Y`; current partial season scored by the champion (OOS by construction). Every row carries `is_oos` + `train_through_season` (downstream eval filters `where is_oos`).
**AC:** ✅ leakage invariant `is_oos ⟺ train_through_season < game_year` enforced + tested (end-to-end walk-forward over synthetic 2018–2025); in-sample seasons excluded from eval by predicate; signals refresh via the Dagster signal phase (9th generator in `daily_ingestion_job`; reported-but-NOT in the freshness HALT floor — nothing consumes it yet, so it must not gate `predict_today`); versioned in the registry.
...
### E2.7 — Distribution UX  🧩  ✅ **SHIPPED 2026-07-27**  **[separate app session — prompt emitted by the E2 model session; see §0.3]**
> ✅ **SHIPPED 2026-07-27:** the totals pick-detail page renders the calibrated per-side NegBin convolution — game-total PMF bell (market line + projected mean + shaded lean), headline tiles (projected total, 80% predictive range, calibrated P(over) = the honest E9.23 CI), run-margin distribution + P(home outscores away), and team-total + alt-line P(over) ladders — beside the SHAP drivers, honest-framing throughout (`best_alpha=0`, never a bet rec). Served from the E2.5 signal (app CONSUMES, never recomputes) via `betting_ml/utils/totals_serving.py` (NEW; the exact-integer PMF fixed the sawtooth) written in the daily `write_serving_store.py --game-detail --s3` blob (WARN-tier), Pydantic-declared on `GameDetailResponse` (`TotalsDistribution`/`DistPmfPoint` — the E9.41 landmine), rendered by `frontend/components/totals-distribution.tsx`. 2026-07-25 persisted to prod; the ~120-date season backfill is the operator's to run AFTER committing (the plausibility guard changed >3→>4 mid-session, so re-run to overwrite stale blobs). ⏭️ operator: `git add betting_ml/utils/totals_serving.py betting_ml/tests/test_totals_serving.py scripts/write_serving_store.py frontend/components/totals-distribution.tsx` (backend models + page + changelog already pushed); Vercel redeploy; then the season backfill loop. Follow-up = E2.8 (per-side μ plausibility) below.
**Scope:** render the predictive total/run-diff distribution + market-line rule + shaded favorable mass + an alt-line ladder, beside the existing per-pick SHAP `pick_explanation` (Story 30.15). Serve via Railway PG (params + quantiles only, never raw samples at request time). **AC:** distribution + drivers render on the pick detail page; honest-framing copy; changelog entry; **the full 2026 season is persisted** — the game-detail blob backfilled for every 2026 date that has per-side μ coverage (2026-03-25 → season-to-date, 120 dates / ~1,488 games as of 2026-07-27; the All-Star break has no games), so historical pick-detail pages render the distribution, not just today-forward. (Backfill is per-date `write_serving_store.py --game-detail --s3 --date <D>` over the covered dates; each daily run then extends it once E2.5's forward-scoring op is live on the box.)
...
> **⚠️ MODEL FOLLOW-UP (E2.5, opened 2026-07-27 from the E2.7 app session):** the served per-side generative μ (`totals_perside_mu_v1`) is **occasionally implausibly low** — ~2.6% of 2026 games (336/13,150) have a side μ < 2.5 full-game runs (e.g. game 824734: home μ 1.29 → convolved total 5.6 vs the champion's 8.6 → two contradictory projected totals on the page). Season avg is a correct 9.07, so it's a per-game quality issue (feature gaps / model extrapolation), not a global bias. **E2.7 mitigates in the app** via `totals_serving.distribution_is_plausible` (suppresses the distribution when a side μ < 2.5 **or** the convolved total diverges > 4 runs from the served champion — the blob omits it, the panel no-ops; floor+>4 hides ~5.3% of games, the two models agree within ~1.09 runs on average). **Durable fix belongs in E2.5/E2.1-r** (a per-side μ floor, or the feature/scoring gap that produces the low estimates). Until then the app guard hides that ~5.3%.

```
▶ App-session prompt — Story E2.7 (Distribution UX)  [app repo · 🧩 APP · ✅ UNBLOCKED — E2.3 served + E2.6 verdict is IN]
✅ GENERATED by the E2 model session (E2.6 final step, 2026-07-26); ✅ READINESS-REFRESHED 2026-07-27.
   Build the predictive-totals distribution surface on the totals pick-detail page. Read §0.2 (app-target
   rule: UI ⇒ frontend/ ONLY; API ⇒ app/backend/) + Story 30.15 (the existing per-pick SHAP `pick_explanation`).
⚠️ E2.6 VERDICT IS NOW KNOWN — a CONFIRMED CLEAN NULL (team_totals 0/239 FDR survivors, alt_totals 0/468;
   placebo indistinguishable; MLB straight/derivative edge exhausted). CONSEQUENCE FOR THIS STORY:
   NO derivative (team-total / alt-line / F5) is surfaced as "+EV" or "value" — the distribution ships as
   PURE TRANSPARENCY / insight ("here is our calibrated view of this total"), never a bet rec. This is
   settled, not "likely." (`best_alpha=0` holds program-wide.)
⚠️ F5 is NOT in scope — E2.5b (F5 registration) is BACKLOGGED; render only the game-total + team-total +
   alt-line ladder from the served per-side NegBin convolution. Do NOT block E2.7 on an F5 distribution.
...
HONEST FRAMING (non-negotiable — best_alpha=0): this is a CALIBRATED DISTRIBUTION for transparency,
  NOT an edge claim. E2.6's derivative gate CLOSED as a CONFIRMED CLEAN NULL (ablation_results/
  e2_6_derivative_gates.md — 0 FDR survivors across team_totals + alt_totals, placebo-indistinguishable),
  so NO market has cleared the gate ⇒ show EVERY derivative as insight/context ("our calibrated total"),
  never "+EV bet" / "value" / a win-rate. There is no actionable-derivative branch to build. Copy is
  descriptive ("we think this game lands around X runs, with this much spread"), not prescriptive.
  SEASON BACKFILL (AC): persist the distribution for the WHOLE 2026 season, not just today — loop
  the per-date game-detail write over every 2026 date with per-side μ coverage (2026-03-25 →
  season-to-date; the All-Star break has no games) so historical pick-detail pages render it:
  `for D in <dates>; do AWS_DEFAULT_REGION=us-east-2 uv run python scripts/write_serving_store.py
  --game-detail --s3 --date $D; done` (laptop; --s3 = DuckDB-over-S3, no Snowflake key). Idempotent —
  overwrites each slate's game-detail blob with the distribution added. TODAY has no μ until E2.5's
  daily forward-scoring op is deployed on the box; every daily run then extends the backfill forward.
  FINAL STEP: add a frontend/data/changelog.json entry (Monday-of-week `week`).
```
...
### E7.1 — MiLB game-log / box ingestion  ✅ **DONE — AC MET 2026-07-24 (backfill 2005→2026: 4.58M player-game-log rows verified at scale)**
**Tasks:**
...
**AC:** ✅ **MET** — per-player MiLB game logs land with level/league/affiliate/park/date/age + leakage-safe `ingestion_ts` (ISO-VARCHAR). **Backfill 2005→2026 complete: 4,582,404 log rows / 177,630 games / 180,307 schedule rows** (AAA 1.67M · AA 1.18M · High-A 1.15M · Single-A 1.13M). Full-scale `delta_scan` read-through PASS — null rates level/league/date/ingestion_ts 0%, park 0.004%, age 0.009%, affiliate 0.171% (correct: Mexican League + MLB rehab/complex entries have no parent org). 2020 correctly absent (COVID); idempotent partition-skip round-trip confirmed. **Freshness op WIRED (2026-07-24):** `milb_incremental_ingest_op` → isolated `milb_ingest_job` (schedule `milb_ingest_job_schedule`, cron `0 13 UTC`, `default_status=RUNNING`) runs `--incremental` daily (current month + prior-month first-3-days), WARN-tier, off the MLB serving path. ⏭️ Goes live on the next box `up -d --build` deploy (baked image).
...
### E7.2 — AAA Statcast ingestion (2023+)  ✅ **DONE — AC MET 2026-07-25**
**Tasks:**
- [x] Pull AAA Statcast (Hawk-Eye parks) where available into a level-tagged Delta table.
**AC:** ✅ **MET, with a live-probed shape correction.** `scripts/ingest_statcast_aaa_to_s3.py` lands `s3://baseball-betting-ml-artifacts/baseball/milb/statcast_aaa` (Delta, partitioned season/level/month, SF-free instance-role S3 auth, idempotent partition-skip). **The public Savant surface for minors turns out to be an aggregate leaderboard builder (`statcast-search-minors/csv`, `group_by=` dropdown), not a pitch-level export** — the MLB `/statcast_search/csv?type=details` endpoint silently ignores minor-league filters and only ever returns MLB rows, so a literal `stg_batter_pitches`-grain match was not achievable from any public source. Landed the finest grain the tool offers instead: `group_by=name-date` → one row per (player_id, game_pk, player_type) of Statcast summary metrics (avg exit velo/launch angle/spin/xwOBA/bat-speed/attack-angle/etc., `avg_`-prefixed for honesty about the game-average grain) — this is exactly E7.1's `player_game_logs` grain and exactly the rate/summary shape E7.3 asked for. Column vocabulary mirrors `ingest_statcast_to_s3.py`'s MLB Statcast naming so metrics read consistently across sources despite the grain difference. `game_pk`/`player_id` verified live against the MLB Stats API (same MLBAM id space as E7.1; a real read-through matched 388/388 statcast game_pks against E7.1's schedule table for a June-2024 slice). EARLIEST_SEASON=2022 (probed — 2019–2021 empty; the true dividing line, not the "2023+" shorthand). Level scope is `AAA`/`A` only — spot-checked directly against `AA`/`A+`/`R`/`ROK`/`DSL` (all zero rows), confirming the site's own dropdown wasn't an incomplete list — `A` supported via `--levels`, not backfilled by default. Coverage-by-venue-by-season report ships as `--coverage-report` (DuckDB `delta_scan` joined against E7.1's schedule table, since the Savant export itself carries no venue field). Tests: `betting_ml/tests/test_ingest_statcast_aaa.py` (17, fast-gate). Dagster: `statcast_aaa_incremental_ingest_op` added as a sibling to E7.1's `milb_incremental_ingest_op` inside the existing `milb_ingest_job` (no new job/schedule).
> **Implementation reality (verified LIVE, not coded to docs):** the minors leaderboard endpoint requires `minors=true` or it 200s with a Statcast-shaped header and ZERO rows every time; `game_date_gt`/`game_date_lt` are unreliable (a 2022 date-bounded request silently returned the whole season) — `hfMo=<month>|` is the reliable filter, which conveniently matches the (season, month) partition grain; the CSV export has an undocumented hard 10,000-row cap (sorted by the request's sort_col desc, so a truncation silently drops the lowest-workload players first) — `fetch_month` logs a loud ALERT at ≥9,000 rows, though every (season, month, player_type) slice probed stayed well under it (peak ~8,203).
> **🔄 CORRECTION — the operator ran the full `--seasons 2022-2026` backfill + `--coverage-report` 2026-07-25 (this session had only verified small slices):** 285,172 rows / 10,390 games / 4,613 players landed, 2022-04-05→2026-07-23. The result REFUTED this story's own initial coverage claim — "2022 = partial rollout (Charlotte/PCL only)" was sourced from a secondhand web-search summary, not the real data, and was **wrong**: the coverage report shows 2022 at 96–100% per-park coverage league-wide, statistically indistinguishable from 2023–2026 (confirmed from Opening Day — 341 distinct April-2022 games across all 30 AAA parks). Removed the `FULL_ROLLOUT_SEASON=2023` constant accordingly — there is no real rollout-completeness dividing line in the data. **The genuine, small (~1–8% per park), structural gap:** Savant's minors leaderboard has ZERO AAA rows for October in every season checked (2022–2025) — the last few regular-season games each year (verified: Las Vegas Ballpark 2022's 3 missing games are exactly its final 3, 2022-09-30/10-01/10-02) simply aren't in this source — plus the 2 known one-off park outliers (Minute Maid Park 2023 / Daikin Park 2025-26, Sugar Land's annual novelty game at the parent club's MLB park). **Lesson: a secondhand web summary is not a substitute for checking the real per-row data at scale — apply the "don't code to docs" discipline to coverage CLAIMS, not just schemas.** ⏭️ operator: box `up -d --build` deploy still pending to bring the Dagster op live.

### E7.3 — Minor→major translation factors (MLEs)  ✅ **DONE — AC MET 2026-07-26 (operator ran the real S3 bake-off; a differentiated per-metric result)**  **[the modeling crux]**
**Tasks:**
- [x] Build level/league run-environment adjustments translating AAA/AA/A wOBA, K%, BB%, ISO (+ AAA Statcast where present) into **MLB-equivalent** rates — a §0.5 bake-off, not a lookup: `partial_pool` (the shared `hierarchical.py` solver, level nested in league, EB-shrunk, boundary-avoiding tau) vs `multiplicative` (Davenport/James factor foil) vs `gbm` (+AAA-Statcast) vs a `level_mean` null, with `identity`/`archetype_prior` reported benchmarks.
- [x] Calibrate on graduated players (pre-debut minor line vs realized MLB line, strictly as-of) — supervised, leave-one-MLB-debut-cohort-out expanding CV, PBO/DSR/oracle-floor guarded.
- [x] Emit a per-(player, level) MLB-equivalent line + PARAMETER uncertainty; active prospects emitted flagged `is_prospect` (the E7.5/E8 deliverable).
**AC:** ✅ **MET.** Operator ran `build_graduated_pairs` (20,453 (player, level) rows; **1,750 labelled graduates** — bridge healthy, not dead — + 10,716 prospects) + the 4-metric bake-off. **The honest, differentiated result — plate DISCIPLINE translates, the composite does not:** **K%** (OOS corr **0.637**, DSR **1.000**, PBO 0.000) and **BB%** (corr **0.491**, DSR **0.989**, PBO 0.000) are STRONG feeders — they clear the strict live-grade deflation bar and beat both the level-mean null and the generic archetype prior. **ISO** (corr **0.429**, DSR 0.679, PBO 0.043) is weak-but-real (beats null + archetype, doesn't clear DSR≥0.95). **wOBA carries NO translatable signal beyond level** (corr 0.220, DSR 0.032) — the winner TIES the level-mean null and the population archetype prior (0.0285 vs 0.0284), so the emission degrades to the population prior. **`partial_pool` (the shared solver) won every metric** over the `multiplicative` Davenport foil (which underperformed the null on wOBA/ISO) and GBM+Statcast (never beat it) — the Bayesian partial-pool form is validated, the classic factor MLE refuted. `best_alpha = 0`; PARAMETER uncertainty. **⇒ E7.5 should wire K%/BB% (and ISO with wide uncertainty), NOT wOBA.** Projections landed at `baseball/milb/derived/mle_projections` (12,398 wide rows). Files: `betting_ml/scripts/milb_mle/{milb_mle,build_graduated_pairs,run_milb_mle}.py`, the shared `betting_ml/utils/hierarchical.py` (promoted from football; football re-exports it), `betting_ml/tests/test_milb_mle.py` (17), `sub_model_registry.yaml::milb_mle_v1`, report `ablation_results/e7_3_milb_mle.md`.

### E7.3p — PITCHER minor→major translation (MLE)  ✅ **DONE — AC MET 2026-07-27 (operator ran the real S3 bake-off same day; a differentiated per-metric result)**  **[the E7.3 sibling; closes the pitcher half of E8.0 + the starter-EB cold-start]**
**Tasks:**
- [x] The SAME shared harness (`milb_mle.py` projectors/bake-off/emission + `betting_ml/utils/hierarchical.py` solver, reused NOT forked) under a pitcher config: `minor_pa` = TBF; metrics per-TBF. New in `milb_mle.py`: `PITCHER_METRICS`, `compute_pitcher_rate_metrics_from_counts`, an `aux_cols` GBM channel (AAA stuff/velo/spin + `minor_start_share` role feature, impute-flagged), `player_type`/`model_version` on `MleConfig` — batter defaults untouched (regression-guarded).
- [x] Pre-registered metrics: **`k_pct`, `bb_pct`** (expected strong — command/whiff), **`hr_rate`** (HR-suppression, expected noisy), **`gb_pct`** (⚠️ cross-definition: MiLB feature = ground-OUT share GO/(GO+AO) from the box line, MLB label = Statcast GB/BIP from `stg_batter_pitches` — the regression learns the rescale, stated), **`xwoba_against`** (feature = E7.2 AAA-Statcast pitcher summaries ONLY, 2022+ — data-thin honest-skip allowed; the composite expected weakest, E7.3's wOBA precedent).
- [x] `build_graduated_pairs_pitchers.py` — SF-free DuckDB/S3 assembly (E7.1 `pit_*` pre-debut per level, E7.2 pitcher summaries TBF-weighted, label = `mart_pitcher_rolling_stats` per-game actuals TBF-weighted over the first 2 MLB seasons `min_mlb_tbf=150`, GB label from `stg_batter_pitches`), assemble ONCE → parquet.
- [x] `run_milb_mle_pitchers.py` — per-metric bake-off (partial_pool@{2,4} / multiplicative / gbm+sc / level_mean null / identity+archetype benchmarks; leave-one-debut-cohort-out expanding CV; PBO/DSR/oracle-floor) → wide projections `mle_projections_pitchers` + report `e7_3p_milb_mle_pitchers.md` incl. **§3 the documented starter-EB rookie-prior path** (the E7.5 pattern: recalibrate → κ-blends for bounded rates → fail-safe view → serving-check; NOT wired this story).
- [x] Tests `betting_ml/tests/test_milb_mle_pitcher.py` (13, fast-gate): pitcher rate math, gate-table completeness, batter-defaults regression, planted-signal bake-off, aux-channel + clone stability, variance-collapse pinning, cohort-leakage tamper, emission semantics, data-thin skip. Registry `sub_model_registry.yaml::milb_mle_pitcher_v1`.
**AC:** ✅ **MET.** Operator ran `build_graduated_pairs_pitchers` (23,862 (pitcher, level) rows; **2,034 labelled graduates** — bridge healthy, > the batter side's 1,750 — + 11,919 prospects; 1,829 w/ AAA-Statcast, 5,795 w/ a GB label) + the 5-metric bake-off. **The honest, differentiated result — batted-ball TILT is the pitcher skill that translates; the composite does not:** **`gb_pct` is the lone STRONG feeder** (OOS corr **0.551**, DSR **1.000**, PBO 0.000 — the cross-definition GO-share→GB/BIP proxy carried cleanly, beating the null 0.0480 vs 0.0586). **`k_pct`/`bb_pct` are weak-but-real** (corr 0.366/0.367, PBO 0.014/0.000, DSR 0.786/0.947 — beat both null and archetype but don't clear DSR≥0.95). ⭐ **Pitcher K% translates FAR more weakly than batter K% (0.366 vs E7.3's 0.637)** — a real asymmetry (same harness, same CV), pitcher outcomes being more role/park/defense-confounded. **`hr_rate` is a TIED-FIELD NULL despite its mechanical 🟡** — beats the null by 1e-4 in a field where winner/archetype/level_mean all sit at 0.0099–0.0100, PBO 0.900, corr 0.094: per the E2.1-r tied-field discipline, effectively NO-SIGNAL (HR outcomes are BABIP-adjacent, as pre-registered). **`xwoba_against` NO-SIGNAL** (ties the archetype prior, DSR 0.030) — the exact mirror of the batter wOBA null. **`partial_pool` won all 5 metrics; `multiplicative` underperformed the null on all 5** — the classic Davenport factor MLE is now refuted on both sides of the ball. `best_alpha = 0`; PARAMETER uncertainty. **⇒ the starter-EB wiring story (the E7.5 pitcher sibling) wires `gb_pct` + `k_pct`/`bb_pct` (wide recalibrated uncertainty), NOT `hr_rate`/`xwoba_against`** (report §3 has the full 5-step path: recalibrate → κ-blends for bounded rates → fail-safe view → serving-check). Projections landed at `baseball/milb/derived/mle_projections_pitchers` (13,892 wide rows — the E8.0 board's pitcher column is populated).

### E7.4 — Prospect identity & ETA xref  ✅✅ **DONE + LANDED + VERIFIED (2026-07-27) — 40,449 rows live; every key match-counted on the REAL lake**
**Tasks:**
- [x] Cross-reference MLBAM / FanGraphs / prospect-list IDs; track level, age, ETA.

**Spine:** MLBAM `person.id` is ONE career-stable id (minors AND majors) ⇒ the spine. FanGraphs fragments: a stable `fg_minor_id` (`minorMasterId`, `sa`-prefixed) + a SEPARATE numeric `fg_mlb_id` assigned at debut. THE BOARD carries **no MLBAM id at all** (column present, 100% NULL), so the prospect→MLBAM bridge routes through E7.7's LEADERBOARD feed, plus an independent graduate leg through the MLB FanGraphs feeds:

    board.fg_minor_id ─HOP1─▶ fg_leaderboards.fg_minor_id ─HOP2─▶ .mlbam_id (xMLBAMID) ─▶ SPINE
    board.fg_player_id (numeric ⇒ fg_mlb_id) ─HOP3─▶ fg_{hitting_leaderboard,stuff_plus}_raw $.playerid → $.xMLBAMID ─▶ SPINE

**Files:** `betting_ml/scripts/milb_xref/player_xref.py` (pure builder + tripwires + report) + `build_player_xref.py` (CLI: `--s3`/`--dry-run`/`--no-enforce`/`--report-json`); `pipeline/ops/milb_ops.py::dim_player_xref_build_op` (WARN-tier, a **downstream fan-in** of all four ingest ops in `milb_ingest_job` — built at job start it would crosswalk the PREVIOUS cycle's feeds, the silent INC-25 class); tests `betting_ml/tests/test_milb_player_xref.py` (15, fast-gate, offline fixtures).
**Storage:** `baseball/milb/derived/dim_player_xref` (Delta, un-partitioned current-state snapshot; point-in-time prospect history stays in `the_board`'s as_of snapshots for E7.8).

**AC:** ✅ **MET + VERIFIED ON THE LANDED TABLE.** `delta_scan` readback OK, **zero void columns** in the output; → MLB player master **6,571/6,571 = 100%**, → E7.3 batter MLE **6,365/6,365 = 100%**, → E7.3p pitcher MLE **7,474/7,474 = 100%**; 40,449 rows / 40,449 distinct `xref_key`. Match rates measured on the live S3 lake (2026-07-27) — HOP 1 **99.3%** current board (99.6–99.9% for 2018–2025); HOP 2 `xMLBAMID` **100.0%** populated and 100% numeric-shaped, landing in a spine source at **100%** for every full-season level (CPX 56.8% / DSL 22.5% = E7.1 sportId-11–14 COVERAGE, not a bad key — those players still get rows with `in_milb_game_logs=false`); HOP 3 graduate leg **97.2%**; **end-to-end current board → MLBAM = 99.3%** ⭐ (the join E8.0 + E7.8 depend on). The 9 unresolved (0.7%) are `level=NULL` pre-pro signees with no MLBAM record anywhere — emitted honestly (`mlbam_id` NULL, `mlbam_match_method='unresolved'`, `xref_key='fg:<id>'`, prospect attributes intact). **No fuzzy leg, deliberately:** the single name-equality hit was a FALSE POSITIVE (prospect "Michael Massey" DET vs MLB 2B Michael Massey, MLBAM 686681, Royals). Provenance stamped on every row (`mlbam_match_method` / `mlbam_match_confidence`); dead-bridge tripwires (floors 0.90/0.95/0.99/0.90) raise rather than ship a near-empty crosswalk, and because the op is WARN-tier a breach leaves the LAST-GOOD xref in place. Full report: `ablation_results/e7_4_prospect_xref.md`.

> 🚨 **TWO LANDMINES FOUND — both would have silently poisoned E8.0.**
> **(1) `delta_scan` CANNOT read `baseball/milb/the_board`.** Its 100%-NULL `mlbam_id` made pyarrow infer the arrow `null` type ⇒ the Delta schema records `"void"` ⇒ DuckDB errors `Unsupported Delta table type: 'void'` on the WHOLE table. Sanctioned reader = `DeltaTable(...).to_pyarrow_dataset()` (`player_xref.register_board`). Root cause fixed at the writer (`ingest_fangraphs_prospects_to_s3.py` now pins string columns via `astype("string")`), but already-written partitions keep the void schema until a full re-backfill. Same class as INC-17: an all-null column silently picks a type nobody wants — pin it at the writer.
> **(2) A `read_parquet('…/the_board/**/*.parquet')` glob reads TOMBSTONED files and FABRICATES a bad match rate.** The 2026-07-27 partition globs to 3,870 rows across three superseded ingest generations vs the Delta-ACID truth of 1,290 — and the superseded generation extracted `fg_minor_id` differently, dragging the *measured* HOP-1 rate from **99.3% to 84.2%**. The first pass of this analysis used a glob and produced exactly that wrong number. **A Delta table has no valid glob reader** (the E11.20 phase-1.5 path-reader class).
...
### E7.7 — FanGraphs prospect ingest (THE BOARD + MiLB leaderboards → `fg_minor_id`)  ✅ **DONE (2026-07-27) — both feeds live on the box; historical backfill in scope**
**Tasks:**
- [x] Ingest FanGraphs **THE BOARD** (FV / rank / ETA / level / org) → S3 Delta, keyed by `fg_minor_id` (+ MLBAM id), with `as_of_date` snapshot dating for leakage-safe as-of joins (E7.8). WARN-tier Dagster op.
- [x] Ingest the FanGraphs **MiLB statistical leaderboards** (batting + pitching) → S3 Delta — the `fg_minor_id` POPULATION feed. **WHY BOTH (operator, 7/27):** THE BOARD only covers RANKED/graded prospects (~1,300); a deep dynasty roster is full of UNRANKED minor leaguers (12-team × 8 minors ⇒ ~100 rostered, most with no Board FV). The leaderboards enumerate EVERY minor leaguer with a stat line → the id coverage E7.4's xref + the E8.0 draft board need so no rostered player is missing.
**Endpoints (probed, not coded-to-docs):** FanGraphs is CF-gated (`cf-mitigated: challenge`, HTTP 403 to any direct client — re-verified live 2026-07-27). Two JSON endpoints, both routed THROUGH the box's **FlareSolverr** via `scripts/utils/fangraphs_client.py`: THE BOARD = `GET /api/prospects/board/data?draft=<season>prospect&season=<yr>` (~1,300 rows; `fetch_prospects_board()`); the MiLB leaderboards = `GET /api/leaders/minor-league/data` (paginated, INC-26-hardened; `fetch_minor_leaderboard()`, `qual=0` everyone, `ind=1` per player-season; note FanGraphs inverts the range params — `season`=END, `season1`=START). Both confirmed against the FanGraphs mobile-app path (`github.com/hedgertronic/fungo`). **⭐ Every row carries BOTH `playerid` (→ `fg_minor_id`) AND `xMLBAMID` (→ the MLBAM id E8.0 joins to the E7.3 MLE projections on)** — no fragile name-match. **Column-name-reality:** every row stores its FULL `raw_json` + typed columns via CASE-INSENSITIVE alias lookup; a `--probe` mode prints the live board's real field names + alias resolution and writes nothing (the ONE real pull confirms the extraction before any write).
**Files:** `scripts/ingest_fangraphs_prospects_to_s3.py` + `scripts/ingest_fangraphs_milb_leaderboards_to_s3.py` (both S3-native Delta, SF-FREE, instance-role auth via `delta_lake.storage_options`, idempotent partitions, `--probe`/`--dry-run`/`--from-csv`), `fetch_prospects_board()` + `fetch_minor_leaderboard()` in `scripts/utils/fangraphs_client.py` (shared `_paginate_leaderboard` helper), `pipeline/ops/milb_ops.py::{fangraphs_prospects_ingest_op, fangraphs_milb_leaderboards_ingest_op}` (WARN-tier) wired into `milb_ingest_job` (daily 13:00 UTC), tests `scripts/tests/test_fangraphs_prospects_ingest.py` (12) + `test_fangraphs_milb_leaderboards_ingest.py` (10).
**Storage:** `s3://baseball-betting-ml-artifacts/baseball/milb/the_board` (one row per as_of_date·season·fg_minor_id) + `.../baseball/milb/fg_leaderboards` (one row per season·stats·as_of_date·fg_minor_id).
**AC:** ✅ **MET on the box (2026-07-27).** BOARD written (1290 rows: fg_minor_id 1286/1290, FV 1290, ETA 1290, org_rank 1290; mlbam_id 0 — the board carries none). LEADERBOARD endpoint CONFIRMED (the minor param set `type=0`/`seasonEnd`/`level`/`splitTeam` → 3345 bat rows; rows DO carry `xMLBAMID` = the MLBAM bridge, so E8.0/E7.4 join on the leaderboard feed, not the board). SF-free confirmed (instance-role S3). **Historical backfill IN SCOPE** (operator 7/27): both scripts take `--seasons 2018-2026` with per-season as-of dating (E7.8 translation research). Manual-CSV fallback (`--from-csv`) available. ⏭️ remaining operator runs: the leaderboard bat+pit write + the `--seasons` historical backfill on both (probe a past season first to confirm the board slug).

### E7.8 — Do FanGraphs prospect rankings translate to MLB projection?  ✅ **DONE — VERDICT REGISTERED 2026-07-27 (operator ran the real S3 assembly + full-grid study)**  **[⛔ UNGATES E8 · 🔬 §0.5 bake-off]**
**📊 THE VERDICT — a DIFFERENTIATED result, not a clean null: FV translates for ARMS, and is redundant with our own MLE for BATS.** 5,245 prospect-seasons / 2,452 players / 5 board cohorts (2018–2022), 3-season outcome windows. Clearing ALL gates (lift>0, BH-FDR q=.10 over the 6-test family, DSR≥0.95, PBO<0.20, oracle floor): **`pitcher/unconditional` lift +0.0411 ρ, p 0.0034, DSR 0.998, PBO 0.000** ⭐ (the survivorship-free headline) and **`pitcher/debut` +0.0268 AUC, p 0.0040, DSR 0.998, PBO 0.000**. Batters: every lift POSITIVE and FDR-surviving on debut/unconditional (+0.0129 / +0.0337, p 0.011 / 0.016) but **failing DSR (0.883 / 0.799) and PBO (0.667 on unconditional, over a TIGHT 0.012 contender spread ⇒ the E2.1-r tied-field read = the NULL)** — under-powered, NOT refuted.
**⭐ THE MECHANISM (report §0b — the question the headline contrast cannot answer): FV and our MLE are SUBSTITUTES for bats and COMPLEMENTS for arms.** Decomposing the same leaderboard — for BATTERS our MiLB performance read adds **+0.047** over the null and FV only **+0.015** on top (the MLE already carries the scouting signal); for PITCHERS our read adds **+0.014** (and −0.015 on the unconditional stage) while FV adds **+0.031** on top. **Independently corroborated by E7.3 vs E7.3p on the same harness: minor-league rates translate far better for bats than arms (batter K% OOS corr 0.637 vs pitcher K% 0.366)** — so the statistical record leaves more unexplained on the pitching side, which is exactly the room a scouting grade fills. The secondary contrast (`null+fv` − `null`) is positive for BOTH types (batter +0.0545 p .041 / pitcher +0.0709 p .006) — FV is informative on its own everywhere; it is only *redundant* for bats.
**⇒ FOR E8:** FV/rank is defensible as a headline ordering input **on the pitching side**; on the hitting side the board should lead with our MLE + age-relative-to-level and treat FV as confirmation, not evidence. ⚠️ Both pre-registered deviations bias TOWARD finding lift, so this POSITIVE verdict carries its caveats (see below); a null would have been the conservative outcome. Report: `ablation_results/e7_8_fv_translation.{md,json}`.
**Tasks:**
- [x] Assemble the prospect→realized-MLB study matrix ONCE → parquet, SF-FREE (`build_fv_cohort.py`): THE BOARD's as-of FV/rank snapshots (E7.7) → the MLBAM spine (E7.4's OWN bridge SQL, reused not re-derived) → the player's MiLB line **strictly before the board date** → the realized MLB outcome over the following N seasons.
- [x] Bake off FV/rank-based projectors against an **age-relative-to-level + level + pedigree** null under purged/embargoed CV, with the survivorship and level confounds MODELLED; deflate over every config tried.
- [x] ✅ **Operator ran both** (2026-07-27): the real S3 assembly (5,252 board rows → 5,245 study rows, 7 unresolved) and the full 33-config study; the verdict is registered.
**The question:** before the Dynasty board (E8) leans on FV/rank as a HEADLINE signal, prove it is signal and not marketing. Dynasty is the GTM lead, so the board's framing must be evidence-based on day one — E7.8 is on E8's critical path (PM/operator 2026-07-23, accepted trade-off). It is also a **direct input to the operator's 2026-08-03 AL-only prospect draft**, and ⭐ **a CLEAN NULL is itself a high-value draft answer** (lean on our MLE + age-relative-to-level; don't overpay for FV hype).
**Target (stated + defended):** the consumers are a dynasty board and a fantasy draft, NOT a front office ⇒ **accumulated points-league fantasy points over the first `--horizon` (default 3) MLB seasons after the board snapshot** — batter `1.3·(H−HR) + 4·HR + 1·BB − 0.5·K`, pitcher `3·IP + 1·K − 1·H − 1·BB − 3·HR` with `IP=(BF−H−BB)/3`. ⭐ **A prospect who never reaches the majors scores ZERO — a realized dynasty outcome, not missing data.** Accumulated (not a rate) because dynasty value is `playing time × quality`; a rate target would hand back the survivorship confound. Limitations stated: R/RBI/SB (and pitcher W/SV/ER) are absent from the Statcast substrate — **SB is a genuinely distinct speed skill this target cannot see**.
**The two confounds, modelled not hand-waved:** **(1) SURVIVORSHIP** — three reported stages separate the channels: `debut` (who ARRIVES, AUC, full cohort = the selection model), `conditional` (production given arrival, ρ — survivorship-EXPOSED by construction, labelled as such, NOT the headline), `unconditional` (⭐ the draft-relevant headline: ρ over the full cohort with zeros kept ⇒ no survivorship selection at all). **(2) LEVEL** — `level` one-hot + **age-relative-to-level** (age minus the TRAIN-fold mean age at that level, fitted in-fold) sit in the NULL arm, so FV is never credited for what level already told us.
**§0.5 bake-off:** 11 pre-registered feature sets (`null` / `null+perf` / `fv_only` / `null+fv` / `null+perf+fv` / `null+rank` / `null+perf+rank` / `null+perf+fv+rank` + a `#bucket` FV-transform variant of each FV-carrying set) × 3 learners (ridge/logistic + 2 GBM settings) = **33 configs, every one counting toward PBO/DSR**. The HEADLINE is a **FIXED PAIR** (`null+perf+fv` − `null+perf`, plus secondary `null+fv` − `null`) so it carries no selection bias; the wide search answers the different question "could a cherry-picked FV config look good by chance?". **The MLE output is deliberately NOT a feature** — E7.3/E7.3p's window is pre-DEBUT, not pre-BOARD, so a graduate's MLE embeds games played after the snapshot (a leak); the as-of raw MiLB line is the leakage-safe stand-in.
**CV + gates:** leave-one-board-cohort-out expanding CV with a **PLAYER PURGE** (a prospect sits on 3–5 consecutive boards sharing ONE overlapping outcome window — without the purge a "projection" is a recollection; purged row counts reported). PBO < 0.20 · DSR ≥ 0.95 · BH-FDR q = 0.10 over the 6-test family (type × stage) · oracle floor. ⭐ **The E2.1-r tied-field read is applied mechanically** — a high PBO over a TIGHT *contender* spread (top quartile, so the crippled `fv_only` arm can't fake a wide field) is the NULL, not overfitting.
**⚠️ Two documented deviations, both biasing TOWARD finding FV lift (so a null is conservative):** (1) **pedigree is a proxy, not draft round/bonus** — MLB draft data is NOT in the lake (no StatsAPI draft ingest exists; that is a real follow-up story), so the null uses `pro_experience_years` + level-for-age and is WEAKER than pre-registered; (2) **pre-2026 as-of is approximate** — FanGraphs serves the RETAINED past board, not a point-in-time snapshot (E7.7 stamps `<season>-07-01`), so a pre-2026 grade may embed hindsight.
**⚠️ Small-N is the defining constraint (the NF1.4 posture):** the fold unit is the BOARD COHORT and a full outcome window costs one cohort per horizon season ⇒ ~5 cohorts / 4 folds, the bare minimum CSCV needs. The study can honestly rule out a LARGE effect, not a small one; where PBO is not computable it says so. `--strict-realtime` runs the stricter "trainable in real time" CV as a sensitivity where folds exist.
**Files:** `betting_ml/scripts/fv_translation/{fv_translation,build_fv_cohort,run_fv_translation}.py` · `betting_ml/tests/test_e7_8_fv_translation.py` (43, fast-gate — including the **assembly SQL EXECUTED against DuckDB fixtures**, which buys down the CI-blind lakehouse-read risk: the as-of guard and the outcome-window bounds are asserted, not hoped for) · `ablation_results/e7_8_fv_translation.{md,json}`.
**🚨 LANDMINE FOUND ON THE FIRST REAL RUN (2026-07-27) — A VENDOR RELABELLED A CATEGORY MID-PANEL AND NO DEFLATION GATE CAN SEE IT.** FanGraphs' board labels arms `RHP`/`LHP` through 2020 and switches to ROLE labels **`SP` / `SIRP` (single-inning reliever) / `MIRP` (multi-inning reliever) from 2021**. A regex written against the old vocabulary typed **666 relievers as BATTERS** (319 in 2021, 346 in 2022) — each then scored on the *batter* fantasy formula, recording ~0 points and a **0.0 debut rate**, so they entered the two most recent eval folds as fake "never arrived" prospects, while the pitcher cohort for those folds collapsed to an **SP-only** (non-random, higher-arrival) subset. The contaminated batter debut rate read 0.226/0.238 for 2021/2022 vs 0.369 in 2020; corrected it is **0.391**, and cohort composition moved 3,307/1,857 → **2,597/2,648**. **⚠️ THE GENERAL LESSON: mislabelled ROWS are wrong BEFORE any model runs, so CV / PBO / DSR / FDR all faithfully deflate a corrupted target and report a confident verdict — this class is catchable ONLY at the data layer.** CURE (shipped): classification cascades to OBJECTIVE evidence — position tokens only when unambiguous, else the **MiLB game logs** (which also resolve the real two-way slash positions `SIRP/SS`, `1B/LHP`, `SS/RHP`), else a *counted* default; an unrecognised token returns "undecided" and defers to the logs instead of silently defaulting to batter; **a tripwire RAISES (`CohortValidationError`) when >5% of a season's rows typed `batter` carry pitcher-majority game logs**; `unknown_position_tokens` is reported every run; and `_derive` is idempotent (`pre_board_mlb_exposure` instead of overwriting `pre_board_mlb_pa`) so `--from-parquet` re-derives in seconds without another lakehouse read. **Whenever a vendor category drives a TARGET or a POPULATION SPLIT, cross-check it against an independent objective source and tripwire the disagreement.**
**AC:** ✅ **MET.** A registered verdict (FV adds deflated, confound-controlled lift for PITCHERS on both the selection and the survivorship-free value channel; a tied-field/under-powered null for BATS), the target stated + defended, the mechanism decomposed (substitutes for bats, complements for arms), and the draft-actionable takeaway spelled out — including what to do for the type that did NOT clear the gates.
