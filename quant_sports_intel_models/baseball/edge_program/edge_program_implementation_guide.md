...
- [x] Emit a per-(player, level) MLB-equivalent line + PARAMETER uncertainty; active prospects (no MLB label yet) emitted flagged `is_prospect` (the E7.5/E8 deliverable).
**AC:** ⚠️ **HARNESS + FAST-GATE (17 tests) GREEN; the validated per-level/-league factors + MLE backtest error on the graduated-player holdout are PENDING the operator's real S3 build + bake-off** (>1-min, SF-free DuckDB/S3 — per the >1-minute + runtime-gate rules a session cannot run the real fit here). Reads E7.1 `player_game_logs` + E7.2 `statcast_aaa` (feature, as-of pre-debut) + `mart_batter_rolling_stats` `_std` (realized MLB label) via DuckDB, joined on the shared MLBAM id. `best_alpha = 0`; uncertainty is PARAMETER uncertainty (E7.5 recalibrates). Files: `betting_ml/scripts/milb_mle/{milb_mle,build_graduated_pairs,run_milb_mle}.py`, the shared `betting_ml/utils/hierarchical.py` (promoted from football; football re-exports it), `betting_ml/tests/test_milb_mle.py`, `sub_model_registry.yaml::milb_mle_v1`, report `ablation_results/e7_3_milb_mle.md`.
