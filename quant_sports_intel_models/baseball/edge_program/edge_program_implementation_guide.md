...
### E7.7 — FanGraphs prospect ingest (THE BOARD + MiLB leaderboards → `fg_minor_id`)  🟡 **CODE-COMPLETE (2026-07-27) — pending the operator's box pulls**
**Tasks:**
...
**AC:** ⏳ `fg_minor_id`-keyed prospect + leaderboard parquet with as-of dating + coverage reports (# ranked, # with MLBAM id, # minor leaguers, seasons) — **emitted on the operator's first box pulls** (`--probe` then a real write; needs `FLARESOLVERR_URL`). SF-free confirmed. Manual-CSV fallback (`--from-csv`) if the endpoint is blocked on 8/3.
