...
### E5.2 — Per-prop distributional pricing  ⬜  **[market-blind model]**
**Tasks:**
- [ ] Price each prop from the player's predictive distribution: `pitcher_outs` → `starter_ip_v1` NegBin (directly); `pitcher_strikeouts` → a starter strikeout distribution; `batter_total_bases`/`hits` → per-batter PA-level outcome distribution from EB wOBA/ISO + matchup + expected PAs.
- [ ] Condition on expected playing time (Story 33.1); reuse E2's distributional machinery.
**AC:** per-prop P(over/under) at the book's line; PIT-calibrated under E1.1 CV.
...
|----|--------|-------|-----|--------|-----------------|
| E9.2 | A0.4.34 | CLV meta-model confidence bar (**H2H only**) per pick | P2 | ✅ SHIPPED 2026-06-18 | H2H CLV bar on the pick detail; **totals excluded** (low-info, per E9.23). Fixed the **NULL-meta-columns** root cause (`arviz`+`h5netcdf` added to the Dockerfile — serve-side trace-load; **sibling of INC-1's `pymc` train gap, still open**) + `MAX() OVER` so morning meta survives to post-lineup rows (`write_serving_store.py`, `picks.py`). CLV framed as line-value leading indicator, not a bet rec |
...
## 5H. Epic E11 — Infrastructure & Cost Savings  🔄  **[cost initiative · execution home for §6]**
