...
| `stg_statsapi_probable_pitchers` | (game_pk, side) | Extracts home/away probable pitchers from schedule response. | table |
...
## 6.8 Temporal Data Platform (Epic 13)

| Gap | Status | Notes |
|---|---|---|
| SCD Type-2 tables | **Missing** | No slowly-changing dimension tables exist anywhere in the project. |
| Point-in-time feature joins (AS OF semantics) | **Missing** | All joins use current state. |
| `feature_ts` / `computed_at` on feature marts | **Missing** | Feature marts have no materialization timestamp. |
| Historical CLV reconstruction infrastructure | **Missing** | No `feature_snapshot_id`, `prediction_ts`, `lineup_state_version` tracking at prediction time. |
| Temporal audit of existing tables for leakage risk | **Not done** | Leakage guards enforced in feature layer but no formal temporal audit of the mart layer exists. |
