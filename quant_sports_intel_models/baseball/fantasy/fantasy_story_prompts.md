...
🟢 **THE ENGINE ALREADY EXISTS — REUSE IT, DON'T REBUILD (NFL MVP-2/NF-C1-lite shipped it 2026-07-24):** `quant_sports_intel_models/fantasy_engine/` is SPORT-AGNOSTIC (`league_config.py` = `LeagueConfig`/`ScoringRules`/`RosterSlot`/**`SportProfile`** + JSON round-trip; `scoring.py` = raw-line rescore + uncertainty passthrough; `vor.py` = greedy flex/superflex allocation → replacement → VOR → ranked board). ⇒ **F-C1 = write a BASEBALL `SportProfile` (column map for F1's stat line + MLB positions) + MLB presets (roto-vs-points cats, keeper depth), instantiate the engine, land a `mart_mlb_fantasy_league_board`.** Study `football/nfl/fantasy/league_presets.py` + `run_league_board.py` as the reference instantiation. ⭐ **A LEAGUE = `(format, size)` two keys — size is a grain dimension (VOR replacement scales with it).** ⚠️ roto (category) scoring is the MLB-specific wrinkle the profile must handle (NFL is points-only) — that's the main net-new logic; the flex/scarcity/VOR core is reused verbatim.
