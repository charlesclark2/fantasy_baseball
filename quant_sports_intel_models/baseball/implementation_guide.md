...
26. **TOTALS PAUSED (Epic 10.6, 2026-06-02):** `total_runs.bet_paused: true` in `model_registry.yaml`. The Epic 19 permission gate must surface NO totals bets — neither v4 nor the Layer 3 challenger beats naive/market on 2026 (regime-adaptation; persistent OVER bias). Un-pause only when a totals model beats naive (Brier < 0.25, alpha-blended) on the current season. Evidence: `ablation_results/totals_2026_failure_analysis.md`.
