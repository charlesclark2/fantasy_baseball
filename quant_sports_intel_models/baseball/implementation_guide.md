...
**Tasks:**
- [ ] **Build the walk-forward OOS scoring path (once):** extend `train_totals` (and the NGBoost champion's training) to persist per-game held-out predictions (prior-season-only models); both models' OOS `p_over`/`mu`/`r` feed every metric below. Do NOT use the all-data refit artifact for the comparison.
...
- [ ] Re-check the **tail over-confidence** found in 10.4 (`[0,0.10)` gap −0.24, `[0.90,1.00]` gap +0.27) on the OOS predictions — if it persists, it constrains Kelly sizing and must be flagged in the promotion decision
