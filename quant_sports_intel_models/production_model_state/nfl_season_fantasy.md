...
### ⚖️ STANDING GOVERNANCE PRINCIPLE — deflation gates bind CLAIMS; measurement gates bind CONSUMPTION (NF-W8 QB Option-B, ratified 2026-08-21)

A **deflation gate** (`dsr_ok` / PBO) asks whether a winner's margin over its own field survives selection bias — the right question before **staking a CLAIM** (an edge, a certification). A `best_alpha=0` **consumer** of a model input makes a *different* claim ("this input is calibrated AND better than the alternative"), which is answered by **PIT + CRPS-vs-matched-foil** (per-fold MEASUREMENTS, not selection statistics), not by deflation. ⇒ a projection consumer may gate on the **measurement** even when the deflation gate fails — but ONLY (1) registered **FORWARD** (adopting it after a failed gate is the E2.1-r inversion / laundering), (2) caveated on every downstream row via a **MECHANICAL guard** (not prose — the NF-C0e "wired ≠ invoked" cure), (3) fully **reversible/deploy-held**, and (4) once the model-layer fixes are **measured-exhausted** (PARK is the wrong disposition when there is no information left to wait for). This is a **scoped** judgment about *which gate binds which consumer* — the `dsr_ok` shipping/certification bar is **UNTOUCHED for everyone else**, and **a future position/target registers its OWN decision** (it never inherits this one). First and only application to date: NF-W8's consumption of the recalibrated QB assembly (`zm_floor`), **APPROVED-WITH-CONDITIONS** by an independent second reader (2026-08-21; provenance/E2.1-r confirmed — the registration predates the decisive run by ~16 h). Full packet + conditions: `docs/nf_w8_qb_option_b_second_reader_memo.md`.
...
| Rookie recalibration | **ON in the served payload** (NF-D16 at λ=0.5 via NF-D21-PUBLISH, SHIPPED 2026-08-21) | code (`serving_lambda()`→0.5) + payload (`rookie_policy`{PM_JUDGMENT, λ 0.5, statistically_selected:false}) |
...
1. **The rookie point level was knowingly COLD** (tier_bias −32…−58 PPR per position; three independent confirmations) — the ratified fix (NF-D16) is now SERVED at λ=0.5 via NF-D21-PUBLISH (SHIPPED 2026-08-21). λ=0.5 is a half-strength board-blind shrink, so ~half the cold-bias correction remains unapplied by construction (a PM-judgment conservatism, not λ=1.0).
...
| **NF-D21 publish-at-λ=0.5** | 2026-08-21 | **✅ SHIPPED + LIVE (PR #984)** — the PM-judgment Route-1 publish, admissible under the NF-D22 power-floor that replaced the coin-toss floor which refused it 08-05. Not a selection; the refusal is preserved beside it | `rookie_publish_policy.py`, catalog:528 |
...
- **Differences-by-decision, NOT mismatches:** (a) NF3.4's transparency block stamps `nfl_fantasy_nf1_v1` (research model, self-labeled); (b) the served rookie point now carries NF-D16's recalibration at a board-blind λ=0.5 (**NF-D21-PUBLISH, SHIPPED + LIVE 2026-08-21**) — a recorded PM-judgment publish under the NF-D22 power-floor; (c) `projections.json`'s stamp intentionally overwrites the fastpath stamp although MVP-1 owns the levels — the two-layer design, documented in the exporter.
