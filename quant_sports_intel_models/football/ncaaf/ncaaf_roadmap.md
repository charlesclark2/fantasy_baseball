...
- **Phase 1B — the NCAAF GAME MODEL (operator-scoped 2026-07-13 — plan it fully BEFORE building; the "don't become a mess" discipline).** Strict dependency order — each story gates the next:
  ...
  1b. **P0.5 — COACHING-CHANGE data (operator-scoped 2026-07-13 — a new HC/OC/DC can flip a team's scheme overnight):** **HEAD COACH is FREE via CFBD `/coaches`** (coach↔school↔year history + per-coach-year **SP+ off/def splits** ⇒ not just a change flag but the coach's PRIOR scheme/quality profile). **OC/DC (coordinators) are a GAP — NOT in CFBD;** the tracker is **FootballScoop** (+ ESPN/CBS), no free API ⇒ scrape / maintain a small table, treat as a best-effort gated add (like NIL-$). ⇒ features: new-HC/OC/DC flags, first-year-at-school, + the new coach's prior SP+ off/def profile.
  2. **P1.1 — ANALYTIC DATA MODELING (the supportable structure — do this BEFORE features).** Turn raw → a proper conformed dimensional model: `dim_team`/`dim_player`/`dim_game`/`dim_conference` + `fact_team_game`/`fact_player_game`/`fact_drive`/`fact_play` + **opponent-adjusted + season/week point-in-time rollups** (as-of-week, leakage-safe). ⭐ **SPORT-TAGGED** (the multi-sport serving decision). This is the layer that keeps it from becoming a mess.
  3. **P1.2 — CONFERENCE / TEAM-STRENGTH MIXED-EFFECTS MODEL (operator-called-out — the right tool for CFB).** A hierarchical partial-pooling model (**team nested in conference**) → opponent-adjusted strength estimates that SHRINK toward conference means. CFB's huge talent disparity + sparse cross-conference schedule (records aren't comparable across conferences) is EXACTLY what random-effects partial pooling is for. Produces a team-strength FEATURE (+ its uncertainty) for P1.3; the NIL/transfer signal enters as a strength covariate.
