...
BREAKS: tuple[Break, ...] = (
    ...
    Break("artifact_path_leg_dropped", _RUNNER,
          '        seeds[seed] = {"n_modules": len(closure), "hits": _hits(closure),\n'
          '                       "artifact_hits": _artifact_hits(closure, root)}',
          '        seeds[seed] = {"n_modules": len(closure), "hits": _hits(closure)}',
          ("test_the_audit_also_scans_for_artifact_path_readers_not_only_imports",
           "test_an_artifact_path_reader_on_the_serving_plane_fails_the_audit"),
          "INC-27 read side: a consumer reading the W6d artifact BY FILENAME with no import edge "
          "is invisible, so the audit passes on a plane that does consume the cells"),
    Break("artifact_scan_control_removed", _RUNNER,
          '    if not ctrl_hits:\n        raise InvalidRun(',
          "    if False:\n        raise InvalidRun(",
          ("test_the_artifact_scan_has_its_own_positive_control_and_raises_when_vacuous",),
          "a token matching nothing makes the artifact scan silently clean for EVERY seed"),
    Break("artifact_hits_ignored_by_the_verdict", _RUNNER,
          '    passes = all(not v["hits"] and not v["artifact_hits"] for v in seeds.values())',
          '    passes = all(not v["hits"] for v in seeds.values())',
          ("test_an_artifact_path_reader_on_the_serving_plane_fails_the_audit",),
          "the artifact leg is computed and REPORTED but never reaches the verdict (wired-not-"
          "invoked, NF-C0e)"),
