...
✅ **DONE 2026-07-23** — `football/nfl/fantasy/{season_projection.py,run_season_projection.py}` + view `mart_nfl_fantasy_season_projection` (tag `nfl_fantasy`) + parquet `nfl/fantasy/derived/season_projections/season=2026/` (S3, Delta; 576 players = 502 vets + 74 rookies). Veteran = realized per-game line shrunk by sample × an EXPECTED-GAMES role estimate (fixes `mart_projections_preseason`'s backup-QB-at-#1 failure — Malik Willis was its #1); rookies = draft-slot production curve × P1A residual (composite-first, bounded). Gate MET: face-valid (Josh Allen/CMC/Bijan/Chase top; Mendoza QB#1 rank-6, J.Love RB#3 elite), coverage 93.1% of ≥4g players, holdout Spearman 0.75–0.78. Report: `ablation_results/nf_fastpath_season_projection.md`. **⏭️ MVP-2/NF-C1 OUTPUT-SCHEMA CONTRACT** (season totals, gsis_id grain): `proj_games`; passing `proj_pass_{att,cmp,yds,td,int}`; rushing `proj_rush_{att,yds,td}`; receiving `proj_{targets,rec,rec_yds,rec_td}`; `proj_fumbles_lost`, `proj_two_pt`(NULL); convenience `proj_fp_{std,half,ppr}` (NF-C1 RESCORES the raw line — do NOT consume these as the league score); uncertainty `fp_ppr_{sd,p10,p90}`, `uncertainty_type`(empirical=vet / parameter=rookie); meta `sport,projection_season,base_season,player_id,player_name,position,team_id,source,is_rookie,draft_overall,confidence,model_version,generated_at`.
