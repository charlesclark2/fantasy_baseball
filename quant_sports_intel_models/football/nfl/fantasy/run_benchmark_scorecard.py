...
def main(argv=None):
    ...
    try:
        out = BS.build_scorecard(con, seasons, args.schema, project_fn=_shipped_projection,
        ...
        if args.forward_season is not None:
            out["forward"] = BS.forward_comparison(con, args.forward_season, args.schema,
                                                   project_fn=_shipped_projection,
    ...
    (_REPORT_DIR / "nf_d3_benchmark_scorecard.json").write_text(json.dumps(out, indent=2, default=float))
    if not args.no_report:
        write_report(out, _REPORT_DIR / "nf_d3_benchmark_scorecard.md", manual_dir=args.manual_dir)

    if (args.s3 or args.lake_root) and out["per_season"]:
        from quant_sports_intel_models.football.nfl.ingest import s3io
        asset = _scorecard_asset(out)
        for y, sub in asset.groupby("season"):
            n = s3io.write_dataframe(sub.assign(season=int(y)), sport="nfl", source="scorecard",
                                     season=int(y), tier="fantasy/benchmarks", local_root=args.lake_root)
            log.info("  landed %d scorecard rows → nfl/fantasy/benchmarks/scorecard season=%d", n, y)
    return 0
