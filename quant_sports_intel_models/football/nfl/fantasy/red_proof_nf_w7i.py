...
BREAKS: tuple[Break, ...] = (
    ...
    Break("negative_delta_beats_foil", _MODULE,
          ...
          "the null state"),
    Break("power_limited_published_for_a_band_decision", _MODULE,
          'if d["state"] == "POWER_LIMITED" and hi_pct is not None and hi_pct < CEILING_BANDS[0]:',
          "if False:",
          ("test_a_ceiling_whose_whole_interval_is_below_the_band_is_not_power_limited",),
          "publishes POWER_LIMITED ('buy more seasons') for a bar the interval already excludes"),
    Break("hand_correction_destroys_the_raw_verdict", _MODULE,
          '"classify_null_raw": {k: v for k, v in d.items() if k != "retest_trigger_note"},',
          '"_discarded": None,',
          ("test_the_hand_correction_retains_the_instruments_raw_verdict",),
          "the instrument's raw verdict is destroyed rather than retained (NF-D20)"),
    Break("hand_correction_swallows_genuine_absence", _MODULE,
          'if d["state"] == "POWER_LIMITED" and hi_pct is not None and hi_pct < CEILING_BANDS[0]:',
          "if hi_pct is not None and hi_pct < CEILING_BANDS[0]:",
          ("test_the_hand_correction_never_swallows_a_decisive_state",),
          "a NEGATIVE point estimate (GENUINE_ABSENCE) is relabelled as merely immaterial"),
    Break("control_reverts_to_the_ratio", _RUNNER,
          "passes = bool(growth is not None and growth >= POSITIVE_CONTROL_MIN_ABS_GROWTH "
          "and widened)",
          "passes = bool(move is not None and move >= POSITIVE_CONTROL_MIN_MOVE_PCT)",
          ("test_the_control_gates_on_the_absolute_advantage_not_the_ratio",),
          "sensitivity read on the ratio the shift itself inflates (understates by construction)"),
    Break("failed_original_clause_deleted", _RUNNER,
          '"legacy_clause": {',
          '"_dropped_clause": {',
          ("test_the_failed_original_control_clause_is_retained_and_still_scored",),
          "the FAILED pre-registered clause is erased instead of retained (NF-D20)"),
    Break("control_evidence_destroyed", _RUNNER,
          '        write_report(out, art.with_suffix(".md"))\n        if not passes:',
          "        if not passes:",
          ("test_the_control_writes_its_evidence_before_it_raises",),
          "the refusal destroys its own evidence (INC-43 salvage)"),
