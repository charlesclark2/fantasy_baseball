...
|---|---|---|---|
| 1 | 🔴 PROD (repo integrity) — ✅ **RESOLVED same day (PR #608)** | **NF-W0's deliverables were DELETED from `dev`** — commit `1f2e7b98` ("Pushing up changes", 2026-08-04, 6 min after the PR #602 merge) removed `weekly_frame.py`, `run_nf_w0_audit.py`, `nf_w_v0_data_audit.md`, `test_nf_w0_weekly_frame.py` (19 RED-proven guards), and the 3 `nf_w0_*` ablation artifacts (1,813 lines) — a stale-working-tree sweep, the exact shared-tree class CLAUDE.md 🌿 warns about. | Operator restored all 7 files (PR #608, 2026-08-05) — verified byte-identical to `99e075c9`; the 19-guard suite re-run green. |
