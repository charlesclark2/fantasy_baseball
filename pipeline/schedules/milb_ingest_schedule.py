...
# E7.1 — daily MiLB incremental ingest.
#
...
# auto-run: the Stats API is FREE (no paid-endpoint / double-ingest concern that keeps the intraday
# CAPTURE schedules STOPPED-on-purpose), the write is idempotent per (season, sport_id, month), it
# is Snowflake-FREE (never wakes the warehouse — E11.20-COST), and the op is WARN-tier so a failure
# degrades quietly and touches nothing MLB-serving.
