...
def ingest_odds_api_odds(context, start):
    if os.getenv("ODDS_API_ENABLED", "false").lower() != "true":
...
@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def ingest_parlay_events(context, start):
    _run_script(context, "parlay_api_ingestion.py", ["events"])


@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def ingest_parlay_canonical_events(context, start):
    _run_script(context, "parlay_api_ingestion.py", ["events-canonical"])


@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def ingest_parlay_odds(context, start):
...
@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def ingest_fangraphs_stuff_plus(context, start):
    if not _is_sunday():
...
@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def ingest_fangraphs_catcher_framing(context, start):
    if not _is_sunday():
...
@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def ingest_fangraphs_hitting_leaderboard(context, start):
...
@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def ingest_transactions(context, start):
...
@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def dbt_daily_build(context, start):
...
@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def ingest_umpires_late(context, start):
    # Retry after dbt-build (~10–11 AM ET) when assignments are reliably posted.
    _run_script(context, "ingest_umpires.py", ["--date", _today()])


@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def dbt_umpire_feature_rebuild(context, start):
    _run_dbt(context, [
        "build",
        "--select",
...
@op(ins={"start": In(Nothing)}, out=Out(Nothing))
def backfill_prediction_log(context, start):
