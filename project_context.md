...
- Deduplication: `UNIQUE (run_date, game_pk)` constraint on `lineup_monitor_state` plus a `NOT EXISTS` guard in the INSERT. Every hourly fire (including no-ops) writes one row to `pipeline_run_log` with `task_name = 'lineup_monitor_proc'`.
  - ⚠️ **HISTORICAL — superseded twice.** (1) This Snowflake task was superseded by the Dagster `lineup_monitor_sensor` (Epic 0.5.7). (2) **E11.24 (2026-08-06):** the live monitor `scripts/lineup_monitor.py` (`task_name = 'lineup_monitor'`, distinct from the proc's `lineup_monitor_proc`) no longer audits to `pipeline_run_log` at all — that INSERT was the last Snowflake statement on the tick and the one paying COMPUTE_WH's provisioning wait, so the audit moved to DynamoDB (`credence-prod-serving-cache`, `pk="ops"`, `sk="lineup_audit#{run_date}#{run_ts}"`). Runbook query: `scripts/daily_run.md` → *Lineup Monitor Architecture*. ⚠️ The table is left in place but is now **DEAD — it has no writer at all**: the legacy task DAG in `scripts/ddl/snowflake_task_dag.sql` is a single chain rooted at `TASK_SAVANT_INGESTION`, `USER_SUSPENDED` since 2026-04-30 (its downstream tasks read `started` but are predecessor-driven with no schedule, so they cannot fire), and `lineup_monitor.py` was the sole remaining writer. **An empty `pipeline_run_log` is NOT an outage signal.**
