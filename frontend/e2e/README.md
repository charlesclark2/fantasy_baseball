...
| `specs/freemium-board.spec.ts` | A **logged-out** visitor gets the WHOLE generic board (rows, real numbers, **zero lock chips**, and no redirect); the free/paid boundary is stated **below** the board and names **both** paid halves; it **links** to the track record without quoting its number; the full-season rate renders **immediately beside** expected points and shows an em-dash for a zero / null / absent games figure; no denied claim anywhere in the rendered DOM; and the paid half (League Board, Draft Optimizer, My Teams) **still bounces** a stranger | The freemium build. The funnel-killing failure — a free visitor REDIRECTED before the board renders — is invisible to every Python assertion in the repo, because the API is perfectly happy and serves the full board to anyone; only a browser following the navigation can see it. The `Infinity` case is the other one nothing else catches: `pts × 17 ÷ 0` is a `number`, so it survives every `!= null` guard and prints "∞" in a points column |
