...
| `specs/home-positioning.spec.ts` | The home page carries a **first-class door to BOTH verticals** (scoped to the cards, not to a link label another block can supply); the live model-vs-market block **follows the server** and is framed as a demonstration rather than a recommendation; an empty read and a FAILED read say different things and **neither can blank the page**; both verticals link to an honest record and the gated one says it is gated; **no forbidden claim anywhere in the rendered document**; the blog is out of the primary nav but still reachable | E9.46 — the pre-story page read as an MLB betting site ("Daily edge, quantified") while Credence had become two products. The tout risk is pre-emptive and is the serious one: `best_alpha = 0`, so a featured pick presented as a recommendation would claim on the front door the advantage six recorded nulls say we do not have |
