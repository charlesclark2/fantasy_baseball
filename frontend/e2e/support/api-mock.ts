...
export const FIXTURES = {
  ...
  //   ncaafSlateMixed                 ⭐ NCAAF-P3.3 — CAPTURED 2026-09-01 from the SAME URL, once
  //                                   NCAAF-ODDS-LIVE's ahead-of-kickoff feed had landed. A REAL
  //                                   in-season Saturday: seven of the eight games carry an
  //                                   `odds_api_live` line and one carries none. It REPLACES the
  //                                   generated `ncaafSlateWithMarket`, which built its market
  //                                   blocks by feeding 2025 closes to the shipping
  //                                   `payloads._market()` — a fixture derived from the transform
  //                                   under test can only confirm the shape we already assumed and
  //                                   can never disconfirm it (E9.64b).
  //                                   ⚠️ ITS ABSENT GAME IS A REFUSAL, not an uncaptured kickoff:
  //                                   `reason = market_snapshot_not_pre_kickoff` — the leakage
  //                                   guard declined a line it could not prove was taken before
  //                                   kickoff. That is a THIRD reason value, and no other fixture
  ...
  ncaafSlateMixed: () => fixture("ncaaf-slate-2026-08-29-mixed.json"),
  ...
  //                                 is realized on a real payload rather than asserted from two.
  //                                 ⚠️ BOTH carry `efficiency` and `splits` as STATED ABSENCES
  //                                 (`no_row_for_this_team_and_season`) — the P1.1 rollups hold no
  //                                 2026 rows yet. That is the real state of the wire.
  //   ncaafTeamPopulated            GENERATED by `build-ncaaf-team-populated.py` running the
  //                                 SHIPPING builder, because the AVAILABLE branch of those two
  //                                 blocks has nothing to capture today — exactly as the market
  //                                 panel's available branch had nothing to capture in P3.2.
  //                                 ⚠️ RETIRE IT on the first re-capture after the 2026 rollups
  //                                 materialize, and re-anchor onto the capture.
  // ⭐ NF-WK-FE1 — the WEEKLY blobs. GENERATED by `build-nfl-weekly.py` running the SHIPPING
  // writer (`weekly_serving.build_players`) and the SHIPPING reducer
  // (`nfl_weekly.public_weekly_payload`), because nothing weekly has ever published and there is
  // therefore nothing to capture: measured 2026-09-13, both free routes answer 404 from the Lambda
  // (the gateway authorizer is already NONE, so the route is reachable and simply empty) and the
  // paid route answers 401 at the gateway.
  //
  // ⚠️ THE FREE BLOB IS THE REDUCER'S OWN OUTPUT, never a hand-deleted copy of the entitled one.
  // An entitlement spec whose "free" fixture was hand-reduced would assert against the fixture
  // author's idea of the paywall rather than against the paywall, which is the one thing these two
  // files exist to tell apart.
  //
  // ⚠️ RETIRE BOTH ON THE FIRST REAL PUBLISH and re-capture raw from the live API (E9.64b): a
  // fixture derived from the writer can only ever confirm the shape we already assumed.
  weeklyManifest: () => fixture("fantasy-nfl-weekly-manifest.synthetic.json"),
  weeklyFree: () => fixture("fantasy-nfl-weekly-players-free.synthetic.json"),
  weeklyEntitled: () => fixture("fantasy-nfl-weekly-players-entitled.synthetic.json"),
  ncaafTeam68: () => fixture("ncaaf-team-68.json"),
  ncaafTeam2449: () => fixture("ncaaf-team-2449.json"),
  ncaafTeamPopulated: () => fixture("ncaaf-team-populated.synthetic.json"),
...
export type MockOptions = {
  ...
  fail?: string[]
  /**
   * ⭐ NF-WK-FE1 — which state the WEEKLY surface is in.
   *
   *   "published" — a built week. THE DEFAULT once the surface exists.
   *   "awaiting"  — nothing built for this week yet: the API answers **404**, which is the ORDINARY
   *                 state of this surface between builds rather than a fault, and is what the live
   *                 API answered on 2026-09-13. A spec asserting the honest empty state needs the
   *                 STATUS, not an empty 200 — the page branches on it, so an empty-200 stand-in
   *                 would leave the branch the page actually takes completely untested.
   *   "failed"    — anything that is not a 404. A DIFFERENT fact ("we could not reach the model")
   *                 that the page states differently; a harness that can only produce the first can
   *                 only ever test half of that.
   */
  ...
  /**
   * ⭐ NF-WVR1 Phase B — `/fantasy/nfl/waiver-pool`. Every body is GENERATED by the SHIPPING route
   * over live served data (`e2e/fixtures/build-waiver-pool.py`); nothing here is authored except
   * the two server states that are pure pass-through of RC1's manifest / the platform flag:
   *   "served"      — rosters refreshed, realized facts verified, ordered within position.
   *   "refused"     — stored rosters truncated → `pool: null` + the named refusal.
   *   "unpublished" — no season artifact → the stated unranked listing.
   *   "excluded"    — "served" plus one RC1 `excluded` week (the route copies that list verbatim).
   ...
   *   "mixed"     — ⭐ THE REALISTIC SATURDAY (NCAAF-P3.3), captured from the same URL on
   *                 2026-09-01: seven games priced off the live ahead-of-kickoff feed and one
   *                 without a line. Both arms of the market render are REALIZED on one payload,
   *                 which is the only way to tell "the panel renders a line" apart from "the panel
   *                 renders whatever it is handed". Supersedes the generated `market` mode.
   ...
   */
  ncaafSlate?: "captured" | "mixed" | "degraded" | "empty" | "failed"
  /**
  ...
  /**
   * ⭐ NCAAF-P3.3 — what `/ncaaf/teams/{id}` answers with.
   *
   *   "captured"  — THE DEFAULT, because it is production: whichever captured team the URL names
   *                 (68 or 2449), each with two blocks stating their own absence. A spec that
   *                 forgets to name a mode gets the state real readers are in.
   *   "populated" — all four blocks AVAILABLE. Nothing on the live wire can produce it today, so it
   *                 is generated from the shipping builder; it is the only route to the efficiency
   *                 and splits render.
   *   "missing"   — the real 404. A team we publish nothing for, which is an ORDINARY answer here
   *                 (a non-FBS id, a season not yet written) and not a fault — which is why it is a
   *                 distinct mode from `failed`.
   *   "failed"    — the read 500s. "Nothing is published" and "we could not reach the model" are
   *                 different facts and the page says so; a harness that could only produce the
   *                 first could only ever test half of that.
   */
  ncaafTeam?: "captured" | "populated" | "missing" | "failed"
}
...
function ncaafPayloadFor(
  ...
  teamMode: NonNullable<MockOptions["ncaafTeam"]>,
  ...
  if (pathname.startsWith("/ncaaf/teams/")) {
    if (teamMode === "failed") return { status: 500, detail: "e2e: ncaaf team read failed" }
    // ⭐ THE REAL 404 AND ITS REAL DETAIL SHAPE. `routers/ncaaf.py` answers an unpublished team
    // with a 404 and the page branches on the STATUS — so the harness has to produce the status,
    // not an empty 200. An empty-200 stand-in would leave the branch the page actually takes
    // completely untested.
    if (teamMode === "missing") {
      return {
        status: 404,
        detail: "No NCAAF team page is published for this team. (team_id=999999)",
      }
    }
    if (teamMode === "populated") return { body: FIXTURES.ncaafTeamPopulated() }
    // ⚠️ KEYED ON THE ID IN THE URL, so a spec that navigates to a team gets THAT team's payload.
    // Answering every id with one fixture would make the header, the record and the conference
    // assertions pass no matter which page the browser was actually on.
    const id = pathname.slice("/ncaaf/teams/".length)
    if (id === "2449") return { body: FIXTURES.ncaafTeam2449() }
    return { body: FIXTURES.ncaafTeam68() }
  }
  if (pathname === "/ncaaf/games") {
    ...
    if (slateMode === "mixed") return { body: FIXTURES.ncaafSlateMixed() }
...
/**
 * ⭐ NF-WK-FE1 — the three WEEKLY reads.
 *
 * Kept OUT of `payloadFor` for the same reason the NCAAF reads are: that function is a pure
 * (path, entitlement) map, and these depend on a per-SPEC mode as well. Returning `undefined` for a
 * path this does not own leaves the rest of the resolution chain untouched.
 *
 * ⭐ IT MODELS BOTH AXES, AND THAT IS THE POINT. The two FREE reads are BYTE-IDENTICAL for every
 * caller — neither handler takes a `Request`, so neither can branch on entitlement — and this
 * resolver reproduces that by IGNORING `entitlement` for them. The PAID read is the opposite: it
 * answers only an entitled caller and otherwise falls through to the harness's 501/unmatched, which
...
function waiverPayloadFor(
  pathname: string,
  mode: NonNullable<MockOptions["waiver"]>,
): { body?: unknown; status?: number; detail?: string } | undefined {
  if (pathname !== "/fantasy/nfl/waiver-pool") return undefined
  if (mode === "notFound") return { status: 404, detail: "League not found" }
  if (mode === "refused") return { body: fixture("fantasy-nfl-waiver-pool-refused.generated.json") }
  if (mode === "unpublished") {
    return { body: fixture("fantasy-nfl-waiver-pool-unpublished.generated.json") }
  }
...
function weeklyPayloadFor(
  ...
  // spec can prove it is reading the real shape rather than a stand-in.
  if (mode === "awaiting") return { status: 404, detail: "Weekly projection not found" }
  if (pathname === "/fantasy/nfl/weekly/manifest") return { body: FIXTURES.weeklyManifest() }
  if (pathname === "/fantasy/nfl/weekly/projections") return { body: FIXTURES.weeklyFree() }
  if (pathname === "/fantasy/nfl/weekly/projections-full") {
    return entitlement === "entitled" ? { body: FIXTURES.weeklyEntitled() } : undefined
  }
  return undefined
}
...
export async function mockApi(page: Page, options: MockOptions = {}): Promise<ApiMock> {
  ...
  const fulfil = async (route: Route, apiPath: string, search: string) => {
    ...
    // ⭐ NF-WVR1 — its own status codes too (a 404 for a league outside the quota).
    const weekly =
      waiverPayloadFor(apiPath, options.waiver ?? "served") ??
      weeklyPayloadFor(apiPath, options.weekly ?? "published", entitlement, options.recap ?? "final")
    if (weekly) {
      if (weekly.status) {
        await route.fulfill({
          status: weekly.status,
          contentType: "application/json",
          body: JSON.stringify({ detail: weekly.detail }),
        })
        return
      }
      await route.fulfill({
        ...
        body: JSON.stringify(options.transform ? options.transform(apiPath, weekly.body) : weekly.body),
      })
      return
    }
